...
<h3>Abordagem Clássica — Administração Científica (Taylor)</h3>
<p><strong>Frederick Taylor</strong> focou na tarefa e no <strong>chão de fábrica</strong> (nível operacional): <strong>organização racional do trabalho (ORT)</strong> — estudo de tempos e movimentos para encontrar "a melhor forma" (<em>the one best way</em>) de executar cada tarefa; <strong>seleção científica do trabalhador</strong> (o homem certo pro lugar certo); <strong>padronização</strong> de métodos, ferramentas e equipamentos; <strong>divisão do trabalho e especialização</strong>; <strong>supervisão funcional</strong> (vários supervisores especializados, cada um numa função, em vez de um único chefe generalista); <strong>incentivos salariais</strong> ligados à produção (prêmios por produtividade). Crítica clássica: visão mecanicista do trabalhador ("homo economicus" — motivado só por dinheiro), ignora aspectos sociais e psicológicos.</p>

<h3>Abordagem Clássica — Teoria Clássica (Fayol)</h3>
<p><strong>Henri Fayol</strong> focou na <strong>estrutura</strong> e na alta administração (visão de cima para baixo, oposta à de Taylor): dividiu as operações da empresa em 6 funções — técnicas, comerciais, financeiras, de segurança, contábeis e <strong>administrativas</strong> (a mais importante, coordena as demais). Definiu o <strong>processo administrativo clássico</strong>: <strong>Prever</strong> (planejar o futuro e traçar o programa de ação), <strong>Organizar</strong> (montar a estrutura material e social), <strong>Comandar</strong> (dirigir e orientar o pessoal), <strong>Coordenar</strong> (ligar, unir e harmonizar todos os atos e esforços) e <strong>Controlar</strong> (verificar que tudo ocorre de acordo com as regras e ordens dadas) — a versão moderna resumida é <strong>PODC</strong> (Planejar, Organizar, Dirigir, Controlar). <strong>14 princípios gerais</strong> mais cobrados: divisão do trabalho, autoridade e responsabilidade, disciplina, <strong>unidade de comando</strong> (cada subordinado recebe ordens de um único superior — princípio mais cobrado em prova), unidade de direção, subordinação do interesse particular ao geral, remuneração, centralização, cadeia escalar (hierarquia), ordem, equidade, estabilidade do pessoal, iniciativa, espírito de equipe.</p>

<h3>Abordagem Humanística (Mayo — Teoria das Relações Humanas)</h3>
<p><strong>Experiência de Hawthorne</strong> (Elton Mayo, fábrica da Western Electric): descobriu que a produtividade não depende só de condições físicas/financeiras, mas principalmente de <strong>fatores psicológicos e sociais</strong> — o <strong>efeito Hawthorne</strong> (trabalhadores produzem mais só por se sentirem observados/valorizados), a importância dos <strong>grupos informais</strong> (normas de comportamento próprias do grupo, distintas da estrutura formal), a liderança informal e as necessidades de reconhecimento e participação. Reação direta à visão mecanicista da Administração Científica: o trabalhador passa a ser visto como "homem social", não apenas "homem econômico".</p>

<h3>Abordagem Neoclássica</h3>
<p>Retoma os princípios clássicos (Fayol) com ênfase prática e nos <strong>resultados</strong>, sem abandonar as contribuições humanísticas. Características: ênfase na prática, reafirmação dos princípios clássicos, ênfase nos objetivos e resultados, <strong>ecletismo</strong> (aproveita contribuições de todas as escolas).</p>
<p><strong>Tipos de organização (decorrência da Teoria Neoclássica)</strong> — a estrutura formal que a empresa adota para distribuir autoridade e trabalho:</p>
<ul>
  <li><strong>Organização linear</strong>: forma mais simples e antiga, baseada na hierarquia militar — autoridade linear e única (cada subordinado se reporta a um único superior), linhas formais de comunicação, centralização das decisões. Vantagem: simplicidade e clareza; desvantagem: rigidez, sobrecarga dos chefes, comunicação lenta.</li>
  <li><strong>Organização funcional</strong>: aplica o <strong>princípio da especialização das funções</strong> (autoridade funcional, dividida por especialidade — o modelo de "supervisão funcional" de Taylor) — cada subordinado pode receber ordens de vários especialistas, cada um em sua área. Vantagem: especialização profunda; desvantagem: diluição e possível conflito de autoridade (subordinado responde a mais de um chefe).</li>
  <li><strong>Organização linha-staff (linha e assessoria)</strong>: combina a <strong>autoridade linear</strong> (órgãos de execução/operação, cadeia de comando única) com a <strong>autoridade de staff</strong> (órgãos de assessoria e consultoria, que aconselham mas não mandam) — é o modelo mais usado hoje, une a unidade de comando da linear com a especialização técnica do staff.</li>
</ul>
<p><strong>Departamentalização</strong>: critério de agrupar atividades e recursos em unidades organizacionais — <strong>funcional</strong> (por função: produção, marketing, finanças, RH), <strong>por produto/serviço</strong>, <strong>geográfica/territorial</strong>, <strong>por clientela</strong>, <strong>por processo</strong>, <strong>por projeto</strong> (equipes temporárias dedicadas a um projeto específico), <strong>matricial</strong> (combina funcional + projeto, com dupla subordinação).</p>
<p><strong>Administração por Objetivos (APO)</strong>: modelo em que superior e subordinado <strong>negociam e definem objetivos conjuntamente</strong>, com prazos e indicadores de desempenho claros — o controle passa a ser feito pelos <strong>resultados alcançados</strong>, não pela supervisão direta do método de trabalho. Ciclo típico: definição conjunta de objetivos → elaboração de planos táticos/operacionais → execução com autonomia → avaliação periódica dos resultados → revisão dos objetivos.</p>

<h3>Modelo Burocrático (Max Weber)</h3>
<p><strong>Weber</strong> descreveu a burocracia como o tipo ideal de organização racional-legal, caracterizada por: <strong>caráter legal das normas</strong> (regras escritas e exaustivas); <strong>caráter formal das comunicações</strong>; <strong>divisão do trabalho</strong> com base na especialização funcional; <strong>impessoalidade nas relações</strong> (o cargo é o que importa, não a pessoa que o ocupa); <strong>hierarquia da autoridade</strong> bem definida; <strong>rotinas e procedimentos padronizados</strong>; <strong>competência técnica e meritocracia</strong> (seleção e promoção por qualificação, não por favoritismo); <strong>especialização da administração</strong> (separação entre propriedade e administração); <strong>profissionalização</strong> dos participantes. <strong>Disfunções da burocracia</strong> (efeitos indesejados quando o modelo é aplicado de forma rígida demais): excesso de formalismo/papelório, resistência a mudanças, despersonalização do relacionamento, categorização como base do processo decisório (decide pelo "manual", não pelo caso concreto), exibição de sinais de autoridade, dificuldade no atendimento a clientes e conflitos com o público.</p>

<h3>Organizações, Sistemas e Métodos (O&M) e Abordagem Sistêmica</h3>
<p><strong>O&M</strong> é a área/função que analisa e propõe melhorias na estrutura organizacional e nos processos de trabalho (fluxos, formulários, rotinas, layout) — ferramentas clássicas incluem organograma (estrutura hierárquica), fluxograma (sequência de um processo) e manual de organização.</p>
<p><strong>Abordagem sistêmica</strong>: trata a organização como um <strong>sistema aberto</strong> em constante interação com o ambiente externo — recebe <strong>entradas</strong> (recursos: pessoas, capital, matéria-prima, informação), realiza um <strong>processo de transformação</strong> e gera <strong>saídas</strong> (produtos, serviços, resultados), com <strong>retroalimentação (feedback)</strong> ajustando o sistema com base nos resultados obtidos e nas mudanças do ambiente. Contrapõe-se à visão de "sistema fechado" das escolas clássicas, que ignoravam a influência do ambiente externo sobre a organização.</p>
<p><strong>Abordagem contingencial</strong>: não existe "a melhor forma única" de organizar (contrário à busca clássica pelo <em>one best way</em>) — a estrutura e as práticas ideais dependem das <strong>circunstâncias (contingências)</strong> específicas de cada organização: tamanho, tecnologia utilizada, ambiente externo (estável × instável) e estratégia adotada.</p>`,
    questoes: []
  },
  {
...
<p><strong>Recrutamento</strong>: <strong>interno</strong> (aproveita talentos já na organização — mais rápido, barato e motivador, mas limita a renovação de ideias) × <strong>externo</strong> (busca candidatos no mercado — traz sangue novo e experiências diferentes, mas é mais caro, demorado e pode gerar frustração interna) × <strong>misto</strong>. <strong>Seleção</strong>: a <strong>entrevista estruturada</strong> é considerada mais confiável que a livre porque se limita a perguntas relevantes pré-definidas, funcionando como previsor eficaz de desempenho e reduzindo viés do entrevistador (menos espaço pra impressões subjetivas e mais padronização entre candidatos). Outras técnicas: provas de conhecimento, testes psicológicos/de personalidade, técnicas de simulação (dinâmicas de grupo, role-playing).</p>
<p><strong>Treinamento e desenvolvimento</strong>: treinamento é focado no <strong>curto prazo</strong> e no cargo atual (habilidades específicas para a função); desenvolvimento tem foco no <strong>longo prazo</strong> e na carreira (preparo para posições futuras, crescimento profissional mais amplo). Etapas do processo: <strong>levantamento de necessidades (LNT)</strong> → planejamento/desenho do treinamento → execução → avaliação de resultados (nível de reação, aprendizado, comportamento no cargo e resultados organizacionais — modelo de Kirkpatrick).</p>
...
<p><strong>Motivação — teorias clássicas</strong>: <strong>Hierarquia de Maslow</strong> (fisiológicas → segurança → sociais → estima → autorrealização, satisfeitas em ordem); <strong>Herzberg</strong> — fatores <strong>higiênicos</strong> (salário, condições de trabalho, relação com chefia: sua ausência gera insatisfação, mas sua presença não motiva, só evita insatisfação) × fatores <strong>motivacionais</strong> (reconhecimento, realização, crescimento: geram motivação de verdade); <strong>Teoria X e Y de McGregor</strong> — Teoria X presume que o trabalhador é preguiçoso e precisa ser controlado; Teoria Y presume que o trabalhador é engajado e busca responsabilidade por natureza.</p>
<p><strong>Liderança — estilos clássicos</strong>: <strong>autocrática</strong> (líder centraliza decisões), <strong>democrática</strong> (decisões compartilhadas com o grupo), <strong>liberal/laissez-faire</strong> (líder delega quase tudo, participação mínima). <strong>Liderança situacional</strong> (Hersey e Blanchard): o estilo ideal depende do nível de maturidade/prontidão do liderado para a tarefa — quanto menos maduro/preparado, mais diretivo deve ser o líder; quanto mais maduro, mais delegador.</p>
...
<p><strong>Ciclo vital dos documentos / Teoria das Três Idades</strong>: <strong>corrente</strong> (documentos em curso, tramitação frequente, uso funcional imediato, mantidos junto aos setores de trabalho); <strong>intermediário</strong> (uso pouco frequente, aguardando prazo prescricional/precaucional em depósito de custódia temporária, ainda podem ser solicitados de volta); <strong>permanente</strong> (valor histórico/probatório/informativo definitivo, guarda por tempo indeterminado, acesso público em regra). A transferência entre idades segue a <strong>tabela de temporalidade</strong>.</p>
<p><strong>Tabela de temporalidade</strong>: documento que define prazos de guarda de cada tipo de documento em cada fase — elaborada por comissões permanentes de avaliação de documentos. A <strong>destinação final</strong> nela prevista tem só duas possibilidades: <strong>eliminação</strong> (descarte definitivo, mediante autorização e registro em termo/edital, quando o documento perde todo valor) ou <strong>recolhimento</strong> (envio ao arquivo permanente/histórico, quando tem valor secundário/histórico) — não se confunde com decisões de suporte (digitalização, microfilmagem) nem com as fases de arquivo corrente/intermediário, que são etapas anteriores à destinação final.</p>
<p><strong>Gestão de documentos</strong> (Lei 8.159/91, Art. 3º): conjunto de procedimentos e operações referentes à produção, tramitação, uso, avaliação e arquivamento em fase corrente e intermediária, visando sua eliminação ou recolhimento para guarda permanente. A lei também estabelece que os arquivos públicos são <strong>acessíveis</strong> em regra (exceto quando seu acesso comprometa a segurança da sociedade/Estado, ou a intimidade/vida privada de pessoas).</p>
<p><strong>Protocolo/triagem de correspondência</strong>: nem todo envelope recebido é aberto — documentos endereçados a um <strong>setor/departamento</strong> (ex.: Departamento Financeiro) ou a canais institucionais como a <strong>Ouvidoria</strong> são abertos, registrados (autuados) e distribuídos pelo protocolo. Já correspondência com <strong>caráter pessoal</strong> (endereçada a um funcionário específico, com informação pessoal) ou material de distribuição geral não individualizado (ex.: periódicos pra distribuição) normalmente <strong>não</strong> é aberta pelo protocolo — só encaminhada fechada ao destinatário. Fluxo típico: recebimento → classificação → registro/autuação → distribuição → movimentação (controle de tramitação) → expedição.</p>
<p><strong>Classificação de documentos</strong>: o método mais moderno é o <strong>funcional</strong> (organiza pelos processos/funções que geram os documentos, refletindo a estrutura organizacional e as atividades do órgão — mais estável no tempo que a classificação por estrutura, que muda a cada reorganização administrativa). Instrumentos de classificação: <strong>plano/código de classificação</strong> (hierarquiza funções, subfunções e atividades) e <strong>quadro de arranjo</strong> (organiza fundos e coleções no arquivo permanente).</p>
<p><strong>Métodos de arquivamento</strong> — escolhidos conforme o tipo de documento: o método <strong>ideográfico</strong> organiza por assunto/tema (indicado quando os documentos têm uma natureza temática comum, como todos os relacionados a títulos e custódia); o método <strong>geográfico</strong> organiza por local (ex.: estado → cidade → correspondente); o <strong>numérico</strong> organiza por número sequencial ou código (simples, cronológico ou dígito-terminal); o <strong>alfabético</strong> organiza pelo nome do interessado/entidade; métodos como <strong>Variadex</strong> e <strong>Soundex</strong> são variações alfabético-numéricas específicas (o Soundex/fonético agrupa nomes com som semelhante, útil para nomes de grafia variável).</p>
...
<p><strong>Suportes e preservação</strong>: <strong>microfilmagem</strong> e <strong>digitalização</strong> são técnicas de reprodução que facilitam acesso e preservação, mas <strong>não substituem</strong> a gestão/avaliação do documento original nem definem, por si só, sua destinação final. <strong>Conservação preventiva</strong>: controle de temperatura/umidade, proteção contra luz e agentes biológicos, manuseio adequado — evita a necessidade de restauração (intervenção mais invasiva e cara, usada quando o dano já ocorreu).</p>
<p><strong>Arquivo público × arquivo privado</strong>: arquivos públicos são os produzidos por instituições públicas nas 3 esferas de governo, e por instituições de caráter público (ex.: entidades privadas encarregadas de serviço público). Arquivos privados podem ser declarados <strong>de interesse público e social</strong> pelo Poder Público, quando seu valor histórico/documental justificar preservação e controle especial (isso não os torna públicos, mas sujeita transferência/alienação a autorização prévia).</p>

<h3>Gêneros, Espécies e Tipos Documentais</h3>
<p><strong>Gênero documental</strong> classifica o documento conforme o sistema de signos utilizado para registrar a informação — refere-se ao <strong>suporte/sistema de signos</strong>, nunca ao conteúdo (um mapa é cartográfico independentemente do assunto que representa):</p>
<div style="overflow-x:auto;margin-bottom:16px;">
<table style="width:100%;border-collapse:collapse;color:#cbd5e1;font-size:0.95em;">
  <thead><tr style="background:#0f172a;">
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Gênero</th>
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Descrição</th>
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Exemplos</th>
  </tr></thead>
  <tbody>
    <tr><td style="padding:8px;border:1px solid #334155;"><strong>Textual</strong></td><td style="padding:8px;border:1px solid #334155;">Linguagem escrita</td><td style="padding:8px;border:1px solid #334155;">Ofícios, relatórios, contratos, atas</td></tr>
    <tr style="background:#0f172a;"><td style="padding:8px;border:1px solid #334155;"><strong>Iconográfico</strong></td><td style="padding:8px;border:1px solid #334155;">Imagens estáticas</td><td style="padding:8px;border:1px solid #334155;">Fotografias, gravuras, desenhos, postais</td></tr>
    <tr><td style="padding:8px;border:1px solid #334155;"><strong>Cartográfico</strong></td><td style="padding:8px;border:1px solid #334155;">Representação espacial</td><td style="padding:8px;border:1px solid #334155;">Mapas, plantas, perfis, cartas geográficas</td></tr>
    <tr style="background:#0f172a;"><td style="padding:8px;border:1px solid #334155;"><strong>Sonoro</strong></td><td style="padding:8px;border:1px solid #334155;">Registros de som</td><td style="padding:8px;border:1px solid #334155;">Discos, fitas cassete, CDs de áudio</td></tr>
    <tr><td style="padding:8px;border:1px solid #334155;"><strong>Audiovisual</strong></td><td style="padding:8px;border:1px solid #334155;">Som e imagem em movimento</td><td style="padding:8px;border:1px solid #334155;">Filmes, vídeos, fitas VHS</td></tr>
    <tr style="background:#0f172a;"><td style="padding:8px;border:1px solid #334155;"><strong>Micrográfico</strong></td><td style="padding:8px;border:1px solid #334155;">Microimagens</td><td style="padding:8px;border:1px solid #334155;">Microfilmes, microfichas</td></tr>
    <tr><td style="padding:8px;border:1px solid #334155;"><strong>Informático</strong></td><td style="padding:8px;border:1px solid #334155;">Meio digital/eletrônico</td><td style="padding:8px;border:1px solid #334155;">Arquivos em CD, DVD, pen drive, nuvem</td></tr>
  </tbody>
</table>
</div>
<p><strong>Espécie documental</strong> é a configuração formal que o documento assume de acordo com a disposição e natureza das informações nele contidas — o <strong>modelo estrutural</strong> do documento (ex.: ata, ofício, decreto, portaria, carta, relatório, certidão, edital, contrato, requerimento, memorando).</p>
<p><strong>Tipo documental</strong> é a espécie documental somada à função/atividade específica que gerou o documento — <strong>Tipo = Espécie + Assunto/Atividade</strong>. Exemplos: ata de posse (espécie ata + atividade posse), ofício de encaminhamento (espécie ofício + atividade encaminhamento), decreto de nomeação (espécie decreto + atividade nomeação), certidão de nascimento (espécie certidão + atividade nascimento).</p>
<p><strong>Ponto crítico para prova</strong>: espécie é o modelo formal genérico; tipo é a espécie aplicada a uma função específica — toda certidão é uma espécie, mas certidão de nascimento é um tipo. Não confundir ainda com <strong>forma</strong> (estágio de preparação do documento: minuta, original, cópia, rascunho) e <strong>formato</strong> (características físicas do suporte: tamanho, forma de apresentação — caderno, rolo, cartão, livro, folha) — nenhum dos dois se refere ao conteúdo ou à função do documento.</p>

<h3>Documentos Digitais e SIGAD</h3>
<p><strong>Documento digital</strong> é a informação registrada em formato codificado digitalmente, acessível por meio de sistema computacional. <strong>Documento nato-digital</strong> é produzido originalmente em meio digital (ex.: e-mail, planilha eletrônica, PDF gerado por computador); <strong>documento digitalizado</strong> é um documento analógico convertido para formato digital via scanner ou fotografia digital. <strong>Ponto crítico para prova</strong>: digitalizar um documento não o transforma em documento nato-digital — a origem determina a classificação, e nem mesmo a aposição de assinatura digital no documento digitalizado muda essa origem.</p>
<p><strong>Metadados</strong> são dados estruturados que descrevem, explicam, localizam ou tornam mais fácil recuperar, usar ou gerenciar um recurso informacional. Em arquivologia, garantem <strong>autenticidade</strong> (registram quem criou, quando, como e por quê), <strong>integridade</strong> (permitem verificar se o documento foi alterado), <strong>confiabilidade</strong> (vinculam o documento ao contexto de produção) e facilitam a <strong>recuperação</strong> (busca e localização eficiente). Classificam-se em: <strong>descritivos</strong> (identificam e localizam o documento — título, autor, data, assunto), <strong>estruturais</strong> (descrevem a organização interna, a relação entre partes do documento), <strong>administrativos</strong> (informações de gestão — direitos de acesso, prazo de guarda, destinação) e <strong>de preservação</strong> (garantem a manutenção do documento ao longo do tempo).</p>
<p><strong>SIGAD (Sistema Informatizado de Gestão Arquivística de Documentos)</strong> é o sistema informatizado que controla o ciclo de vida dos documentos arquivísticos digitais e convencionais desde a produção até a destinação final. Suas funções: capturar documentos (registro e ingresso no sistema), classificar conforme o plano de classificação, controlar temporalidade e destinação (eliminação ou recolhimento), garantir autenticidade e integridade, controlar acesso e tramitação, e preservar documentos digitais ao longo do tempo.</p>
<p><strong>e-ARQ Brasil</strong> é o Modelo de Requisitos para Sistemas Informatizados de Gestão Arquivística de Documentos, publicado pelo <strong>CONARQ</strong>, definindo os requisitos mínimos que um SIGAD deve atender — abrange funcionalidades do sistema, metadados obrigatórios, requisitos de segurança e preservação. <strong>Ponto crítico para prova</strong>: o e-ARQ Brasil não é um software — é um conjunto de requisitos que qualquer sistema deve cumprir para ser considerado um SIGAD adequado.</p>
<p><strong>Assinatura digital</strong> é o mecanismo criptográfico que garante autoria e integridade de documento eletrônico. A <strong>ICP-Brasil</strong> (Infraestrutura de Chaves Públicas Brasileira) é o sistema que credencia autoridades certificadoras para emissão de certificados digitais com validade jurídica no Brasil.</p>

<h3>Instrumentos de Descrição (Arquivo Permanente)</h3>
<p>Os instrumentos de descrição tornam os acervos dos arquivos permanentes acessíveis ao público e aos pesquisadores, elaborados seguindo a <strong>NOBRADE</strong> (Norma Brasileira de Descrição Arquivística).</p>
<p><strong>Guia</strong>: instrumento mais abrangente e genérico — descreve o conjunto de fundos de um ou mais arquivos, orientando o pesquisador sobre o que existe e como acessar. Nível de detalhe baixo; não descreve documentos individualmente, apenas conjuntos de fundos.</p>
<p><strong>Inventário</strong>: descreve as séries e subséries de um fundo ou coleção, em nível intermediário de detalhe — mais específico que o guia, menos que o catálogo. Pode ser <strong>sumário</strong> (descrição mínima) ou <strong>analítico</strong> (descrição mais detalhada por unidade de arquivamento), dando visão estruturada do conteúdo de um fundo.</p>
<p><strong>Catálogo</strong>: descreve individualmente cada unidade documental (documento a documento), com o nível de detalhe mais alto entre os instrumentos. Pode abranger um fundo inteiro ou apenas parte dele (catálogo seletivo), com o objetivo de localizar e identificar documentos específicos com precisão.</p>
<p><strong>Repertório (ou índice)</strong>: lista remissiva que indica onde encontrar determinados documentos ou informações dentro de outros instrumentos de descrição — não descreve os documentos, apenas remete a eles, funcionando como instrumento auxiliar dos demais.</p>
<div style="overflow-x:auto;margin-bottom:16px;">
<table style="width:100%;border-collapse:collapse;color:#cbd5e1;font-size:0.95em;">
  <thead><tr style="background:#0f172a;">
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Instrumento</th>
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Abrangência</th>
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Nível de detalhe</th>
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Unidade descrita</th>
  </tr></thead>
  <tbody>
    <tr><td style="padding:8px;border:1px solid #334155;"><strong>Guia</strong></td><td style="padding:8px;border:1px solid #334155;">Múltiplos fundos / arquivo inteiro</td><td style="padding:8px;border:1px solid #334155;">Baixo</td><td style="padding:8px;border:1px solid #334155;">Fundo</td></tr>
    <tr style="background:#0f172a;"><td style="padding:8px;border:1px solid #334155;"><strong>Inventário</strong></td><td style="padding:8px;border:1px solid #334155;">Um fundo ou coleção</td><td style="padding:8px;border:1px solid #334155;">Médio</td><td style="padding:8px;border:1px solid #334155;">Série / subsérie</td></tr>
    <tr><td style="padding:8px;border:1px solid #334155;"><strong>Catálogo</strong></td><td style="padding:8px;border:1px solid #334155;">Parte ou todo de um fundo</td><td style="padding:8px;border:1px solid #334155;">Alto</td><td style="padding:8px;border:1px solid #334155;">Documento individual</td></tr>
    <tr style="background:#0f172a;"><td style="padding:8px;border:1px solid #334155;"><strong>Repertório</strong></td><td style="padding:8px;border:1px solid #334155;">Auxiliar (remissivo)</td><td style="padding:8px;border:1px solid #334155;">—</td><td style="padding:8px;border:1px solid #334155;">Referência a outros instrumentos</td></tr>
  </tbody>
</table>
</div>
<p><strong>Ponto crítico para prova</strong>: guia = visão geral de tudo; inventário = estrutura do fundo; catálogo = documento por documento; o repertório não descreve — remete.</p>

<h3>Legislação Arquivística — Lei nº 8.159/91 (aprofundamento)</h3>
<p>A Lei nº 8.159/91 (Lei de Arquivos) é a norma geral da política nacional de arquivos públicos e privados. <strong>Arquivos públicos</strong> (Art. 2º): conjuntos de documentos produzidos e recebidos por órgãos públicos federais, estaduais, do DF e municipais, em decorrência de suas funções administrativas, legislativas e judiciárias. <strong>Arquivos privados</strong>: conjuntos de documentos produzidos ou recebidos por pessoas físicas ou jurídicas de direito privado. Compete ao Poder Público (Art. 1º) a gestão documental e a proteção dos documentos de arquivo como instrumentos de apoio à administração, à cultura, ao desenvolvimento científico e como elementos de prova e informação.</p>
<p><strong>CONARQ (Conselho Nacional de Arquivos)</strong>: órgão colegiado, vinculado ao Arquivo Nacional (Ministério da Justiça e Segurança Pública), composto por representantes de órgãos públicos federais, estaduais, municipais e de entidades privadas ligadas à área arquivística. Competências: definir a política nacional de arquivos públicos e privados; expedir normas sobre gestão arquivística (como o e-ARQ Brasil e a NOBRADE); estimular o processo de melhoria do acesso aos arquivos; zelar pelo cumprimento das normas arquivísticas; coordenar o SINAR; e promover a elaboração de instrumentos de gestão (tabela de temporalidade, plano de classificação). <strong>Ponto crítico para prova</strong>: o CONARQ é órgão normativo e de coordenação — não executa diretamente a custódia de documentos; o Arquivo Nacional é o órgão executor central.</p>
<p><strong>Sanções e penalidades (Art. 25)</strong>: a lei responsabiliza quem destruir, inutilizar ou deteriorar documentos de valor permanente ou de interesse público; alienar documentos públicos sem autorização; ou subtrair, esconder ou desviar documentos públicos. As sanções são de três naturezas: <strong>civil</strong> (reparação dos danos causados ao erário e ao patrimônio documental), <strong>penal</strong> (tipificação como crime contra a Administração Pública — Arts. 305 a 311 do Código Penal, como falsidade documental e destruição de documento) e <strong>administrativa</strong> (processo administrativo disciplinar para servidores públicos envolvidos). <strong>Ponto crítico para prova</strong>: a Lei nº 8.159/91 não estabelece multas fixas — remete às responsabilidades civil, penal e administrativa previstas na legislação específica aplicável a cada caso.</p>

<h3>SINAR — Sistema Nacional de Arquivos</h3>
<p>Instituído pelo <strong>Decreto nº 4.073/2002</strong>, que regulamentou a Lei nº 8.159/91, o SINAR tem como objetivo implementar a política nacional de arquivos públicos e privados definida pelo CONARQ, por meio da integração e articulação das atividades arquivísticas de todos os órgãos e entidades da Administração Federal.</p>
<p><strong>Composição do SINAR</strong>: o Arquivo Nacional (órgão central do sistema); os arquivos do Poder Executivo, Legislativo e Judiciário Federal; os arquivos estaduais e do Distrito Federal dos Poderes Executivo, Legislativo e Judiciário; os arquivos municipais dos Poderes Executivo e Legislativo; e os arquivos das universidades e demais instituições públicas federais.</p>
<p><strong>Arquivo Nacional</strong> é o órgão central do SINAR — compete a ele orientar normativamente os integrantes do sistema, prestar assistência técnica e supervisionar as atividades arquivísticas no âmbito federal.</p>
<p><strong>Funcionamento</strong>: os órgãos integrantes do SINAR devem assegurar a gestão documental em sua esfera; identificar, preservar e disponibilizar os documentos de valor permanente; implementar os instrumentos de gestão definidos pelo CONARQ (plano de classificação, tabela de temporalidade); e encaminhar ao Arquivo Nacional os documentos de valor permanente da esfera federal após o prazo de guarda intermediária.</p>
<div style="overflow-x:auto;margin-bottom:16px;">
<table style="width:100%;border-collapse:collapse;color:#cbd5e1;font-size:0.95em;">
  <thead><tr style="background:#0f172a;">
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Instância</th>
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Papel</th>
    <th style="padding:8px;border:1px solid #334155;color:#38bdf8;">Natureza</th>
  </tr></thead>
  <tbody>
    <tr><td style="padding:8px;border:1px solid #334155;"><strong>CONARQ</strong></td><td style="padding:8px;border:1px solid #334155;">Define política nacional de arquivos</td><td style="padding:8px;border:1px solid #334155;">Normativo / deliberativo</td></tr>
    <tr style="background:#0f172a;"><td style="padding:8px;border:1px solid #334155;"><strong>SINAR</strong></td><td style="padding:8px;border:1px solid #334155;">Executa a política por meio dos órgãos integrantes</td><td style="padding:8px;border:1px solid #334155;">Operacional / sistêmico</td></tr>
    <tr><td style="padding:8px;border:1px solid #334155;"><strong>Arquivo Nacional</strong></td><td style="padding:8px;border:1px solid #334155;">Órgão central do SINAR, custódia dos documentos federais permanentes</td><td style="padding:8px;border:1px solid #334155;">Executor central</td></tr>
  </tbody>
</table>
</div>
<p><strong>Ponto crítico para prova</strong>: CONARQ define a política (normativo); SINAR é o sistema que executa essa política (operacional); Arquivo Nacional é o órgão central do SINAR (executor federal).</p>`,
    questoes: []
  },
  {
...
<p><strong>Grupos de processos (ciclo de vida do gerenciamento, PMBOK)</strong>: <strong>Iniciação</strong> (autoriza formalmente o projeto ou uma fase, produz o TAP); <strong>Planejamento</strong> (define e refina objetivos, detalha o curso de ação necessário); <strong>Execução</strong> (realiza o trabalho definido no plano); <strong>Monitoramento e Controle</strong> (acompanha, revisa e regula o progresso e desempenho, identifica desvios, executado em paralelo com as demais fases); <strong>Encerramento</strong> (finaliza formalmente o projeto/fase, aceitação do produto, lições aprendidas).</p>
<p><strong>Termo de Abertura do Projeto (TAP)</strong>: documento produzido na fase de <strong>iniciação</strong> que formaliza as necessidades do negócio, a compreensão das necessidades do cliente, e o produto/serviço que será o resultado do projeto, além de designar formalmente o gerente do projeto — não confundir com o Plano de Gerenciamento do Projeto (mais detalhado, vem depois, no planejamento).</p>
<p><strong>Estrutura Analítica do Projeto (EAP/WBS)</strong>: diagrama hierárquico que decompõe o trabalho do projeto em pacotes de trabalho menores e gerenciáveis, garantindo que <strong>todo o escopo</strong> do trabalho esteja identificado e definido dentro de uma estrutura — não é organograma da equipe nem indica responsáveis por pacote (isso é a <strong>matriz de responsabilidades/RACI</strong>) nem mostra caminho crítico (isso é o cronograma/rede do projeto).</p>
<p><strong>Áreas de conhecimento (PMBOK)</strong>: <strong>Integração</strong> (unifica e coordena todos os processos e atividades); <strong>Escopo</strong> (garante que o projeto contempla todo o trabalho necessário, e só ele); <strong>Cronograma/Tempo</strong>; <strong>Custos</strong>; <strong>Qualidade</strong>; <strong>Recursos</strong> (organiza e gerencia a equipe); <strong>Comunicações</strong>; <strong>Riscos</strong>; <strong>Aquisições</strong>; <strong>Partes interessadas (stakeholders)</strong> — identificação, análise de expectativas e engajamento ao longo de todo o projeto.</p>
<p><strong>Ferramentas de cronograma</strong>: <strong>Gráfico de Gantt</strong> (barras horizontais mostrando início/fim de cada atividade ao longo do tempo — visual, simples, mas não mostra bem dependências complexas entre tarefas); <strong>Método do Caminho Crítico (CPM)</strong> (identifica a sequência de atividades que determina a duração mínima total do projeto — atraso em qualquer atividade do caminho crítico atrasa o projeto inteiro; atividades fora do caminho crítico têm "folga"); <strong>PERT</strong> (técnica de estimativa de duração usando 3 cenários — otimista, mais provável, pessimista — para lidar com incerteza, muito usada junto com o CPM).</p>
<p><strong>Linha de base</strong> (ex.: no MS Project): "fotografia" do planejamento original aprovado (escopo, prazo, custo), usada depois para comparar o andamento real do projeto com o que foi previsto originalmente — mudanças na linha de base só devem ocorrer via processo formal de controle de mudanças.</p>
<p><strong>Estruturas organizacionais e projetos</strong>: <strong>funcional</strong> (projeto subordinado aos departamentos funcionais, gerente de projeto com pouca ou nenhuma autoridade); <strong>projetizada</strong> (equipe dedicada exclusivamente ao projeto, gerente com autoridade quase total); <strong>matricial</strong> (fraca, balanceada ou forte, conforme o equilíbrio de autoridade entre o gerente funcional e o gerente de projeto) — quanto mais "projetizada" a estrutura, maior a autoridade do gerente de projeto e a dedicação exclusiva da equipe.</p>`,
    questoes: []
  },
  {
...
<p><strong>Missão, Visão e Valores</strong>: <strong>missão</strong> é a razão de existir da organização hoje (o que ela faz, para quem, por quê); <strong>visão</strong> é onde a organização quer chegar no futuro (aspiração de longo prazo); <strong>valores</strong> são os princípios éticos que orientam o comportamento e as decisões da organização.</p>
<p><strong>Análise SWOT (FOFA)</strong>: cruza o ambiente <strong>interno</strong> — <strong>Forças</strong> (Strengths) e <strong>Fraquezas</strong> (Weaknesses), controláveis pela organização — com o ambiente <strong>externo</strong> — <strong>Oportunidades</strong> (Opportunities) e <strong>Ameaças</strong> (Threats), fora do controle direto da organização. Usada para formular estratégias que potencializam forças/oportunidades e mitigam fraquezas/ameaças.</p>
<p><strong>5 Forças de Porter</strong> (análise da competitividade de um setor): rivalidade entre concorrentes existentes, poder de barganha dos fornecedores, poder de barganha dos clientes, ameaça de novos entrantes, ameaça de produtos/serviços substitutos.</p>
<p><strong>Matriz GUT</strong>: prioriza problemas/ações combinando <strong>Gravidade</strong> (impacto se não for resolvido), <strong>Urgência</strong> (prazo disponível para agir) e <strong>Tendência</strong> (se o problema tende a piorar com o tempo) — cada critério pontuado (ex.: 1 a 5), multiplicados entre si para gerar a prioridade.</p>
<p><strong>Eficiência, eficácia e efetividade</strong> — não confundir: <strong>eficiência</strong> é fazer bem feito com o menor uso de recursos (meios); <strong>eficácia</strong> é atingir o objetivo/meta estabelecido (fins), independente do custo; <strong>efetividade</strong> é o impacto real e duradouro gerado na sociedade/público-alvo — a dimensão mais difícil de medir, mas a mais relevante para saber se algo realmente mudou.</p>
...
<p><strong>Comportamento organizacional, cultura e liderança</strong>: <strong>cultura organizacional</strong> é o conjunto de valores, crenças e comportamentos compartilhados que orientam a forma de agir da organização (mais estável e profunda que o clima, que é a percepção do momento); <strong>comportamento organizacional</strong> estuda como indivíduos e grupos agem dentro das organizações; <strong>gestão de conflitos</strong> — estilos de resolução vão desde a evitação/acomodação (mais passivos) até a competição e a colaboração (mais assertivos), sendo a colaboração o estilo que busca solução ganha-ganha.</p>

<h3>Gestão da Qualidade</h3>
<p><strong>Ciclo PDCA</strong> — método de melhoria contínua: <strong>Plan</strong> (planejar — identificar o problema, definir metas e plano de ação), <strong>Do</strong> (executar o que foi planejado, em pequena escala se possível), <strong>Check</strong> (verificar/checar os resultados obtidos contra o planejado), <strong>Act</strong> (agir — padronizar o que deu certo, corrigir desvios, reiniciar o ciclo). É a base de qualquer processo de melhoria contínua (Kaizen).</p>
<p><strong>5S</strong> (metodologia japonesa de organização do ambiente de trabalho): <strong>Seiri</strong> (utilização — separar o necessário do desnecessário), <strong>Seiton</strong> (organização — um lugar para cada coisa), <strong>Seiso</strong> (limpeza), <strong>Seiketsu</strong> (padronização — manter as 3 práticas anteriores), <strong>Shitsuke</strong> (disciplina — tornar hábito permanente).</p>
<p><strong>5W2H</strong>: ferramenta de planejamento de ação que responde <strong>What</strong> (o quê), <strong>Why</strong> (por quê), <strong>Where</strong> (onde), <strong>When</strong> (quando), <strong>Who</strong> (quem), <strong>How</strong> (como) e <strong>How much</strong> (quanto custa) — garante que nenhuma dimensão relevante da ação fique sem definição.</p>
<p><strong>Ferramentas da qualidade</strong>: <strong>Diagrama de Ishikawa (espinha de peixe/causa-e-efeito)</strong> — organiza possíveis causas de um problema em categorias (método, mão de obra, material, máquina, meio ambiente, medida); <strong>Diagrama de Pareto</strong> — baseado na regra 80/20 (80% dos problemas vêm de 20% das causas), prioriza onde agir primeiro; <strong>Histograma</strong> (distribuição de frequência de dados); <strong>Fluxograma</strong> (sequência visual de um processo); <strong>Folha de verificação/checklist</strong>; <strong>Diagrama de dispersão</strong> (relação entre duas variáveis).</p>
...
<p><strong>Reengenharia × Melhoria Contínua</strong>: reengenharia é a <strong>reformulação radical e drástica</strong> de processos (recomeçar do zero, mudança abrupta), enquanto melhoria contínua/Kaizen é o aperfeiçoamento <strong>gradual e incremental</strong> — abordagens opostas em intensidade e velocidade de mudança.</p>
<p><strong>Benchmarking</strong>: processo sistemático de comparar processos/práticas/desempenho da organização com os de referências de mercado (concorrentes ou não), para identificar oportunidades de melhoria — pode ser interno (entre unidades da própria organização), competitivo (com concorrentes diretos), funcional (com organizações de outros setores que executam processo semelhante) ou genérico.</p>
<p><strong>Gestão da Qualidade Total (TQM) e normas ISO</strong>: TQM é a filosofia de gestão em que a qualidade é responsabilidade de <strong>todos</strong> na organização, com foco no cliente e melhoria contínua — não é responsabilidade só de um departamento de controle de qualidade isolado. A <strong>família ISO 9000</strong> estabelece normas internacionais de sistema de gestão da qualidade, com certificação atestando que a organização segue processos padronizados e documentados.</p>
<p><strong>Principais teóricos da qualidade</strong>: <strong>Deming</strong> (criador/difusor do PDCA, ênfase em dados estatísticos e melhoria contínua); <strong>Juran</strong> (qualidade como "adequação ao uso", trilogia da qualidade: planejamento, controle e melhoria); <strong>Crosby</strong> ("zero defeito", qualidade é gratuita — o custo está em não fazer certo da primeira vez); <strong>Ishikawa</strong> (diagrama de causa-e-efeito, círculos de controle de qualidade, qualidade como responsabilidade de todos).</p>`,
    questoes: []
  },
  {
...
<p><strong>Reforma do Estado — não confundir os modelos</strong>: <strong>agência executiva</strong> é a qualificação dada a uma autarquia ou fundação <strong>já existente</strong> da administração direta, via contrato de gestão, pra ganhar mais autonomia e controle por resultados; <strong>agência reguladora</strong> é uma autarquia especial criada pra regular e fiscalizar um setor econômico específico (energia, telecomunicações); <strong>organização social (OS)</strong> é uma qualificação dada a entidade <strong>privada sem fins lucrativos</strong> que celebra contrato de gestão pra prestar serviços de interesse público (saúde, educação, cultura, pesquisa); <strong>OSCIP</strong> é outra qualificação de entidade privada, com <strong>termo de parceria</strong> (regras um pouco diferentes da OS); <strong>consórcio público</strong> é uma associação entre entes federativos, não uma transformação de órgão único.</p>
...
<p><strong>Avaliação de políticas públicas — dimensões que não se confundem</strong>: <strong>eficácia</strong> mede se as metas/produtos planejados foram entregues; <strong>eficiência</strong> (ou custo-eficiência) relaciona os resultados obtidos com os recursos gastos; <strong>efetividade</strong> mede o impacto real e a transformação duradoura na vida do público-alvo (ex.: renda, qualidade de vida, negócios que continuam funcionando meses depois) — é a dimensão mais "de longo prazo" e mais difícil de medir, mas a mais relevante pra saber se a política realmente mudou alguma coisa. <strong>Ciclo de políticas públicas</strong>: identificação do problema/agenda → formulação → decisão → implementação → monitoramento → avaliação.</p>
<p><strong>Governança pública e accountability</strong>: <strong>governança</strong> envolve os arranjos institucionais, mecanismos de liderança, estratégia e controle usados para avaliar, direcionar e monitorar a atuação da gestão. <strong>Accountability</strong> é a responsabilização dos agentes públicos por seus atos, com prestação de contas e possibilidade de sanção — <strong>accountability vertical</strong> (eleitores/sociedade cobrando diretamente, via eleições ou controle social) × <strong>accountability horizontal</strong> (órgãos de controle do próprio Estado fiscalizando uns aos outros, ex.: Tribunais de Contas, Ministério Público, Judiciário).</p>
...
<p><strong>Controle da Administração Pública</strong>: <strong>interno</strong> (exercido pelo próprio Poder dentro de sua estrutura — controladorias, auditorias internas) × <strong>externo</strong> (exercido por outro Poder ou órgão distinto — Congresso Nacional com auxílio do Tribunal de Contas, Judiciário, Ministério Público). Quanto ao momento: <strong>prévio/a priori</strong> (antes do ato, ex.: parecer prévio de licitação), <strong>concomitante</strong> (durante a execução, ex.: fiscalização de obra em andamento) e <strong>posterior/a posteriori</strong> (após o ato, ex.: julgamento de contas, auditoria).</p>`,
    questoes: []
  },
  {
...
<p><strong>Remédios constitucionais</strong> — não confundir: <strong>Habeas corpus</strong> protege liberdade de locomoção (ilegalidade/abuso de poder); <strong>Habeas data</strong> garante acesso/retificação de informações do próprio impetrante em banco de dados público; <strong>Mandado de segurança</strong> protege direito líquido e certo não amparado por HC/HD, contra ato de autoridade; <strong>Mandado de injunção</strong> supre ausência de norma regulamentadora que inviabilize direito constitucional; <strong>Ação popular</strong> protege patrimônio público/moralidade administrativa, ajuizável por qualquer cidadão — quando o ato questionado é de autarquia federal, corre na Justiça Federal.</p>
<p><strong>Administração direta e indireta</strong>: direta são os órgãos integrados à estrutura da União/Estados/DF/Municípios (ministérios, secretarias); indireta são as entidades com personalidade jurídica própria criadas para descentralizar a execução de atividades — <strong>autarquia</strong> (pessoa jurídica de direito público, criada por lei específica, ex.: INSS, IBAMA), <strong>fundação pública</strong> (patrimônio personalizado para fim específico, pode ser de direito público ou privado conforme a lei instituidora), <strong>empresa pública</strong> (capital 100% público, qualquer forma societária, ex.: Caixa Econômica Federal) e <strong>sociedade de economia mista</strong> (capital misto com maioria de votos públicos, sempre sociedade anônima, ex.: Banco do Brasil, Petrobras).</p>
<h3>Teoria da Constituição e Aplicabilidade das Normas</h3>
<p><strong>Poder Constituinte</strong>: <strong>originário</strong> (inicial, ilimitado juridicamente, autônomo e incondicionado — cria a nova ordem constitucional rompendo com a anterior; é considerado permanente, pois nunca se esgota, ficando latente e pronto para se manifestar de novo numa ruptura futura) e <strong>derivado</strong> (instituído pelo originário, por isso tem natureza jurídica e é limitado, condicionado e secundário), que se subdivide em: <strong>reformador</strong> (poder de emendar a Constituição, Art. 60 — sujeito a limitações formais/procedimentais, como o quórum de 3/5 em dois turnos em cada Casa; circunstanciais — vedada emenda na vigência de intervenção federal, estado de defesa ou estado de sítio; e materiais — as <strong>cláusulas pétreas</strong> do Art. 60, §4º: forma federativa de Estado, voto direto/secreto/universal/periódico, separação dos Poderes, direitos e garantias individuais); <strong>decorrente</strong> (poder dos Estados-membros de elaborar suas próprias Constituições estaduais, observando os princípios e limites da CF/88 — por simetria, aplica-se de forma semelhante à elaboração da Lei Orgânica do DF e dos Municípios); e <strong>difuso</strong> (manifesta-se através das mutações constitucionais, ver abaixo — não altera o texto, só o sentido da norma). <strong>Supraconstitucionalidade</strong>: teoria (minoritária no Brasil) segundo a qual haveria normas ou princípios hierarquicamente acima da própria Constituição — não confundir com os tratados internacionais de direitos humanos aprovados pelo rito do Art. 5º, §3º (mesmo quórum de emenda constitucional), que ficam no MESMO nível hierárquico da Constituição, e não acima dela.</p>
<p><strong>Classificação das normas constitucionais quanto à eficácia</strong> (classificação de José Afonso da Silva, a mais cobrada): <strong>eficácia plena</strong> — aplicabilidade direta, imediata e integral desde a promulgação, não dependem de lei regulamentadora e não podem ser restringidas por norma infraconstitucional (ex.: Art. 2º, separação dos Poderes); <strong>eficácia contida</strong> (ou de aplicabilidade prontamente restringível) — também têm aplicabilidade direta e imediata, mas <strong>podem</strong> ser restringidas por lei infraconstitucional, por outra norma constitucional, ou por conceitos ético-jurídicos indeterminados (ex.: livre exercício de profissão, Art. 5º, XIII, "atendidas as qualificações profissionais que a lei estabelecer"); <strong>eficácia limitada</strong> — não produzem todos os seus efeitos desde logo, dependem de lei integradora/regulamentadora, subdividindo-se em normas de <strong>princípio institutivo</strong> (traçam o esquema geral de organização de órgãos/entidades, aguardando lei que lhes dê estrutura definitiva) e de <strong>princípio programático</strong> (fixam programas/diretrizes a cumprir pelo Estado, ex.: direito à saúde e à educação como metas de ação). Pegadinha recorrente: mesmo sem regulamentação, as normas de eficácia limitada já produzem efeitos mínimos — revogam normas anteriores incompatíveis, impedem a edição de normas em sentido contrário e servem de parâmetro de interpretação.</p>
<p><strong>Mutações constitucionais</strong>: mudança informal do sentido ou do alcance de uma norma constitucional, <strong>sem alteração do seu texto</strong> — ocorre por via de interpretação (sobretudo pelo STF), pelos costumes, ou por novas práticas políticas; é a manifestação concreta do poder constituinte difuso. Diferente da reforma (emenda constitucional), que altera o <strong>texto</strong> através do procedimento formal do Art. 60 — a mutação altera apenas o <strong>significado</strong> da norma, sem tocar nas palavras, e tem como limite não poder desnaturar o sistema constitucional nem servir para burlar o processo formal de emenda (não pode ser usada para driblar cláusula pétrea).</p>
<h3>Direitos Sociais, Nacionalidade e Direitos Políticos</h3>
<p><strong>Direitos sociais</strong> (Art. 6º ao 11 — direitos fundamentais de 2ª dimensão, de natureza prestacional): o Art. 6º elenca educação, saúde, alimentação, trabalho, moradia, transporte, lazer, segurança, previdência social, proteção à maternidade e à infância e assistência aos desamparados. <strong>Saúde</strong> (Art. 196 a 200): direito de todos e dever do Estado, garantido mediante políticas sociais e econômicas, com acesso universal e igualitário — o SUS é regido pelos princípios da descentralização, atendimento integral e participação da comunidade; a saúde é livre à iniciativa privada, que pode participar do SUS de forma complementar, vedada a destinação de recursos públicos para auxílios/subvenções a instituições privadas com fins lucrativos. <strong>Educação</strong> (Art. 205 a 214): direito de todos e dever do Estado e da família, visando ao pleno desenvolvimento da pessoa, seu preparo para a cidadania e sua qualificação para o trabalho — o ensino fundamental é obrigatório e gratuito, inclusive para os que não tiveram acesso na idade própria; o ensino é livre à iniciativa privada, atendidas as normas gerais da educação nacional e sujeito a autorização e avaliação de qualidade pelo Poder Público. <strong>Previdência social</strong> (Art. 201-202): regime contributivo e de filiação obrigatória, de caráter solidário, organizado sob a forma de regime geral, cobrindo eventos como incapacidade, desemprego involuntário, idade avançada, morte e maternidade. <strong>Liberdade sindical</strong> (Art. 8º — muito cobrada): livre associação profissional/sindical, vedada a exigência de autorização estatal para fundar sindicato (só registro no órgão competente) e vedada qualquer interferência/intervenção do Poder Público na organização sindical; a participação dos sindicatos nas negociações coletivas de trabalho é obrigatória; vigora o princípio da <strong>unicidade sindical</strong> — é vedada a criação de mais de uma organização sindical, em qualquer grau, representativa de categoria profissional/econômica, na mesma base territorial (que não pode ser inferior à área de um Município); o aposentado filiado tem direito a votar e ser votado nas organizações sindicais.</p>
<p><strong>Nacionalidade</strong> (Art. 12 e 13) — vínculo jurídico-político entre o indivíduo e o Estado. <strong>Brasileiro nato</strong>: os nascidos na República Federativa do Brasil, ainda que de pais estrangeiros, desde que estes não estejam a serviço de seu país (jus soli, com exceção); os nascidos no estrangeiro, de pai brasileiro ou mãe brasileira, desde que qualquer um deles esteja a serviço da República Federativa do Brasil (jus sanguinis + critério funcional); os nascidos no estrangeiro de pai brasileiro ou mãe brasileira, desde que registrados em repartição brasileira competente, ou que venham a residir no Brasil e optem, a qualquer tempo depois de atingida a maioridade, pela nacionalidade brasileira (opção confirmativa, ato personalíssimo exercido perante juiz federal). <strong>Brasileiro naturalizado</strong>: os que, na forma da lei, adquiram a nacionalidade brasileira — exigidos dos originários de países de língua portuguesa apenas 1 ano de residência ininterrupta e idoneidade moral; e os estrangeiros de qualquer nacionalidade residentes no Brasil há mais de 15 anos ininterruptos e sem condenação penal, que requeiram a nacionalidade (naturalização extraordinária). <strong>Cargos privativos de brasileiro nato</strong> (Art. 12, §3º): Presidente e Vice-Presidente da República, Presidente da Câmara dos Deputados, Presidente do Senado Federal, Ministro do STF, carreira diplomática, oficial das Forças Armadas e Ministro de Estado da Defesa. <strong>Perda da nacionalidade</strong> (Art. 12, §4º): terá cancelada a naturalização, por sentença judicial, quem exercer atividade nociva ao interesse nacional; perderá a nacionalidade quem adquirir outra, ressalvados o reconhecimento de nacionalidade originária pela lei estrangeira e a imposição de naturalização, pela norma estrangeira, ao brasileiro residente em Estado estrangeiro, como condição para permanência ou exercício de direitos civis.</p>
<p><strong>Direitos políticos e partidos políticos</strong> (Art. 14 a 17): a soberania popular é exercida pelo sufrágio universal e pelo voto direto e secreto, com valor igual para todos, e, nos termos da lei, mediante <strong>plebiscito</strong> (consulta popular prévia, antes do ato legislativo/administrativo), <strong>referendo</strong> (consulta posterior, que ratifica ou rejeita ato já praticado) e <strong>iniciativa popular</strong> (projeto de lei subscrito por, no mínimo, 1% do eleitorado nacional, distribuído por pelo menos 5 Estados, com não menos de 0,3% dos eleitores de cada um deles). <strong>Capacidade eleitoral ativa</strong>: alistamento e voto são obrigatórios para os maiores de 18 anos; facultativos para analfabetos, maiores de 70 anos e maiores de 16 e menores de 18 anos; não podem se alistar como eleitores os estrangeiros e, durante o serviço militar obrigatório, os conscritos. <strong>Inelegibilidades</strong>: são inelegíveis os inalistáveis e os analfabetos; a reeleição é permitida para Presidente, Governadores, Prefeitos e respectivos vices, por um único período subsequente; para concorrer a outros cargos, esses chefes do Executivo devem renunciar até 6 meses antes do pleito. <strong>Perda e suspensão dos direitos políticos</strong> (Art. 15 — é vedada a <strong>cassação</strong> de direitos políticos, mas admite-se sua perda ou suspensão): cancelamento de naturalização por sentença transitada em julgado (perda); incapacidade civil absoluta (suspensão); condenação criminal transitada em julgado, enquanto durarem seus efeitos (suspensão); recusa de cumprir obrigação a todos imposta ou prestação alternativa, nos termos do Art. 5º, VIII (perda); improbidade administrativa, nos termos do Art. 37, §4º (suspensão).</p>
<h3>Controle de Constitucionalidade</h3>
<p><strong>Controle preventivo × repressivo</strong>: o <strong>preventivo</strong> ocorre antes da entrada em vigor da norma, durante o processo legislativo — realizado principalmente pelas Comissões de Constituição e Justiça de cada Casa do Congresso (controle político) e pelo veto jurídico do Presidente da República por inconstitucionalidade (Art. 66, §1º); excepcionalmente o Judiciário atua de forma preventiva, por mandado de segurança impetrado por parlamentar (só por parlamentar) para garantir o devido processo legislativo, quando há afronta direta a cláusula pétrea ou ao próprio rito de emenda. O <strong>repressivo</strong> ocorre depois que a norma já está em vigor — regra geral no Brasil é o controle repressivo judicial, exercido de forma difusa ou concentrada; excepcionalmente há controle repressivo político, como a sustação, pelo Congresso Nacional, de atos normativos do Executivo que exorbitem do poder regulamentar ou dos limites de delegação legislativa (Art. 49, V).</p>
<p><strong>Controle difuso</strong> (via de exceção/incidental): exercido por qualquer juiz ou tribunal, incidentalmente, dentro de um caso concreto, como questão prejudicial — seus efeitos são, em regra, <strong>inter partes</strong> (só atinge as partes do processo) e <strong>ex tunc</strong> (retroativos). <strong>Cláusula de reserva de plenário</strong> (Art. 97): os tribunais só podem declarar a inconstitucionalidade de lei ou ato normativo do Poder Público pelo voto da <strong>maioria absoluta</strong> de seus membros ou dos membros do respectivo órgão especial — nunca por turma ou câmara isolada. <strong>Papel do Senado Federal</strong> (Art. 52, X): compete privativamente ao Senado suspender a execução, no todo ou em parte, de lei declarada inconstitucional por decisão definitiva do STF em controle <strong>difuso</strong> — amplia os efeitos de inter partes para <strong>erga omnes</strong> (o Senado não revoga a lei, só suspende sua execução, por resolução, ato discricionário quanto à conveniência/oportunidade de editá-la, mas vinculado quanto ao conteúdo).</p>
<p><strong>Controle concentrado</strong> (via de ação/abstrato): exercido originariamente pelo STF (lei federal/estadual em face da CF) ou pelos Tribunais de Justiça (lei estadual/municipal em face da Constituição Estadual), fora de um caso concreto, tendo a própria (in)constitucionalidade como objeto principal — efeitos, em regra, <strong>erga omnes</strong>, <strong>ex tunc</strong> (salvo modulação de efeitos pelo STF, por maioria de 2/3, por segurança jurídica ou excepcional interesse social) e <strong>vinculantes</strong> para os demais órgãos do Judiciário e para a Administração Pública direta e indireta. Ações do controle concentrado federal: <strong>ADI</strong> (Ação Direta de Inconstitucionalidade) — ataca lei ou ato normativo federal/estadual em face da CF; <strong>ADC</strong> (Ação Declaratória de Constitucionalidade) — busca confirmar a constitucionalidade de lei/ato normativo <strong>federal</strong> já existente diante de controvérsia judicial relevante (ADI e ADC têm <strong>caráter dúplice</strong>: a improcedência de uma equivale à procedência da outra); <strong>ADPF</strong> (Arguição de Descumprimento de Preceito Fundamental) — cabível quando não houver outro meio eficaz de sanar a lesividade (princípio da subsidiariedade), podendo questionar até lei <strong>municipal</strong> e direito pré-constitucional; <strong>ADO</strong> (Ação Direta de Inconstitucionalidade por Omissão) — combate a inércia do Poder competente em editar norma regulamentadora necessária para tornar efetiva norma constitucional de eficácia limitada — julgada procedente, o STF dá ciência ao órgão omisso e, tratando-se de órgão administrativo, fixa prazo de 30 dias para saná-la.</p>
<h3>Organização do Estado e Repartição de Competências</h3>
<p><strong>Organização político-administrativa</strong> (Art. 18): compreende a União, os Estados, o Distrito Federal e os Municípios, todos <strong>autônomos</strong> nos termos da Constituição (autonomia, não soberania — só a República Federativa do Brasil, como um todo, é soberana no plano internacional). O DF acumula as competências legislativas reservadas aos Estados e aos Municípios, sendo vedada sua divisão em Municípios.</p>
<p><strong>Repartição de competências</strong> — segue o princípio da predominância do interesse (geral/nacional → União; regional → Estados; local → Municípios). <strong>Competências materiais</strong>: <strong>exclusivas da União</strong> (Art. 21, indelegáveis — ex.: manter relações com Estados estrangeiros, emitir moeda, explorar serviços de telecomunicações e energia elétrica); <strong>comuns</strong> a todos os entes (Art. 23, cumulativas/paralelas — ex.: cuidar da saúde, proteger o meio ambiente, combater a poluição e as causas da pobreza); <strong>dos Municípios</strong> (Art. 30 — legislar sobre assuntos de interesse local, suplementar a legislação federal e estadual no que couber, instituir e arrecadar seus tributos, organizar e prestar os serviços públicos de interesse local, incluído o transporte coletivo, manter programas de educação infantil e ensino fundamental, prestar serviços de saúde e promover o adequado ordenamento territorial). <strong>Competências legislativas</strong>: <strong>privativa da União</strong> (Art. 22, delegável a Estados por lei complementar quanto a questões específicas — ex.: direito civil, penal, comercial, do trabalho, processual, eleitoral, agrário); <strong>concorrente</strong> entre União, Estados e DF (Art. 24 — a União estabelece normas gerais e os Estados exercem competência suplementar; inexistindo lei federal de normas gerais, os Estados exercem competência legislativa plena, e a superveniência de lei federal <strong>suspende</strong> a eficácia da lei estadual no que lhe for contrário, sem revogá-la); <strong>remanescente/reservada dos Estados</strong> (Art. 25, §1º — são reservadas aos Estados as competências que não lhes sejam vedadas pela Constituição).</p>
<h3>Poder Legislativo e Judiciário (Aprofundamento)</h3>
<p><strong>Imunidades parlamentares</strong> (Art. 53) — protegem o exercício do mandato, não são privilégio pessoal. <strong>Imunidade material (inviolabilidade)</strong>: Deputados e Senadores são invioláveis, civil e penalmente, por quaisquer de suas opiniões, palavras e votos, mesmo fora do recinto do Congresso, desde que relacionados ao exercício do mandato. <strong>Imunidade formal (processual)</strong>: desde a expedição do diploma, não poderão ser presos, salvo em flagrante de crime inafiançável (caso em que os autos vão, em 24 horas, à respectiva Casa, para que, pelo voto da maioria de seus membros, resolva sobre a prisão); recebida denúncia contra parlamentar por crime ocorrido após a diplomação, o STF dá ciência à Casa respectiva, que, por iniciativa de partido nela representado e pelo voto da maioria de seus membros, pode sustar o andamento da ação até a decisão final do processo (o que também suspende a prescrição enquanto durar o mandato).</p>
<p><strong>CPIs</strong> (Art. 58, §3º): criadas pela Câmara e pelo Senado, em conjunto ou separadamente, mediante requerimento de 1/3 de seus membros, para apuração de fato determinado e por prazo certo, com conclusões encaminhadas ao Ministério Público quando for o caso. <strong>Poderes de investigação</strong> próprios das autoridades judiciais: podem determinar diretamente (sem autorização judicial) a quebra de sigilo bancário, fiscal e telefônico (dos <strong>registros</strong>, não da interceptação em tempo real) e a condução coercitiva de testemunhas que se recusem a comparecer. <strong>Limites</strong> (cláusula de reserva de jurisdição): CPI <strong>não</strong> pode determinar interceptação telefônica (escuta em tempo real), busca e apreensão domiciliar, nem decretar prisão, salvo flagrante ou desobediência/desacato ocorridos na própria sessão.</p>
<p><strong>Órgãos do Poder Judiciário</strong> (Art. 92): STF, CNJ, STJ, Tribunais Regionais Federais e Juízes Federais, Tribunais e Juízes do Trabalho, Tribunais e Juízes Eleitorais, Tribunais e Juízes Militares, e Tribunais e Juízes dos Estados e do DF/Territórios. <strong>STJ</strong>: órgão de cúpula da Justiça Comum (federal e estadual), responsável por uniformizar a interpretação da legislação federal <strong>infraconstitucional</strong> (não a constitucional, papel do STF); composto por no mínimo 33 Ministros, escolhidos entre brasileiros com mais de 35 e menos de 70 anos, de notável saber jurídico e reputação ilibada. <strong>Justiça Federal</strong> (TRFs e Juízes Federais): julga as causas em que a União, entidade autárquica ou empresa pública federal forem interessadas como autoras, rés, assistentes ou oponentes (exceto falência, acidentes de trabalho e as sujeitas à Justiça Eleitoral/Trabalhista), além de crimes políticos e infrações penais em detrimento de bens/serviços/interesse da União. <strong>Justiça Estadual</strong>: competência residual, julgando o que não for atribuído expressamente às demais Justiças.</p>
<p><strong>Garantias dos magistrados</strong> (Art. 95): <strong>vitaliciedade</strong> — adquirida após 2 anos de exercício, quando o juiz passa a só perder o cargo por sentença judicial transitada em julgado (nos primeiros 2 anos, a perda pode se dar por deliberação do tribunal a que estiver vinculado); <strong>inamovibilidade</strong> — não pode ser removido/promovido contra sua vontade, salvo por interesse público, mediante decisão da maioria absoluta do respectivo tribunal ou do CNJ, assegurada ampla defesa; <strong>irredutibilidade de subsídio</strong> — proteção contra redução nominal (não impede perda de poder de compra pela inflação, nem incidência de tributos e descontos legais).</p>
<p><strong>CNJ — Conselho Nacional de Justiça</strong> (Art. 103-B): órgão de controle interno do Judiciário, <strong>sem</strong> função jurisdicional — não julga processos nem revê decisões judiciais. <strong>Composição</strong>: 15 membros, mandato de 2 anos, admitida 1 recondução, presidido pelo Presidente do STF, incluindo membros de outros tribunais, do Ministério Público, advogados (indicados pela OAB) e cidadãos de notável saber jurídico (indicados pela Câmara e pelo Senado). <strong>Competências</strong>: controlar a atuação administrativa e financeira do Judiciário e o cumprimento dos deveres funcionais dos juízes; zelar pela autonomia do Judiciário e pela observância do Art. 37; apreciar, de ofício ou mediante provocação, a legalidade dos atos administrativos praticados por membros/órgãos do Judiciário, podendo desconstituí-los, revê-los ou fixar prazo para que se adotem providências; rever processos disciplinares de juízes julgados há menos de 1 ano.</p>
<h3>Finanças Públicas e Orçamento (Art. 165 a 169)</h3>
<p><strong>Ciclo orçamentário — as 3 leis</strong> (todas de iniciativa privativa do Poder Executivo): <strong>PPA</strong> (Plano Plurianual) — estabelece, de forma regionalizada, diretrizes, objetivos e metas para as despesas de capital e outras delas decorrentes e para os programas de duração continuada, com vigência de 4 anos (do 2º ano de um mandato até o 1º ano do mandato seguinte, garantindo continuidade entre governos); <strong>LDO</strong> (Lei de Diretrizes Orçamentárias) — compreende as metas e prioridades da Administração Pública, orienta a elaboração da LOA, dispõe sobre alterações na legislação tributária e estabelece a política de aplicação das agências financeiras oficiais de fomento; <strong>LOA</strong> (Lei Orçamentária Anual) — compreende o orçamento fiscal, o orçamento de investimento das estatais e o orçamento da seguridade social, estimando a receita e fixando a despesa para o exercício seguinte. As 3 leis se harmonizam em cascata: o PPA orienta a LDO, que orienta a LOA — nenhum investimento cuja execução ultrapasse um exercício financeiro pode começar sem prévia inclusão no PPA.</p>
<p><strong>Princípios orçamentários</strong> mais cobrados: <strong>unidade</strong> (o orçamento deve ser uno, um documento único por ente); <strong>universalidade</strong> (a LOA deve conter todas as receitas e despesas de todos os Poderes, órgãos, fundos e entidades da administração direta e indireta); <strong>anualidade/periodicidade</strong> (o orçamento vigora por um exercício financeiro, coincidente com o ano civil); <strong>exclusividade</strong> (a LOA não pode conter dispositivo estranho à previsão de receita e fixação de despesa, salvo autorização para créditos suplementares e operações de crédito); <strong>não afetação das receitas de impostos</strong> (Art. 167, IV) — é vedada a vinculação de receita de impostos a órgão, fundo ou despesa, ressalvadas a repartição constitucional de receitas tributárias, a destinação a ações e serviços de saúde, a manutenção e desenvolvimento do ensino, e a garantia/contragarantia à União e pagamento de dívida para com esta.</p>
<p><strong>Vedações em matéria orçamentária</strong> (Art. 167): é vedado iniciar programas não incluídos na LOA; realizar despesas que excedam os créditos orçamentários; realizar operações de crédito que excedam o montante das despesas de capital (regra de ouro); e transpor, remanejar ou transferir recursos de uma categoria de programação para outra sem autorização legislativa. <strong>Despesas com pessoal</strong> (Art. 169 + Lei de Responsabilidade Fiscal): não podem exceder os limites da lei complementar — a LRF fixa 50% da Receita Corrente Líquida para a União e 60% para Estados/DF/Municípios; excedido o limite, o percentual excedente deve ser eliminado nos períodos seguintes, com medidas como redução de cargos em comissão, exoneração de servidores não estáveis e, em último caso, exoneração de servidores estáveis.</p>
<h3>Defesa do Estado e das Instituições Democráticas (Art. 136 a 142)</h3>
<p><strong>Estado de Defesa</strong> (Art. 136): decretado pelo Presidente da República, ouvidos o Conselho da República e o Conselho de Defesa Nacional (pareceres não vinculantes), para preservar ou restabelecer, em locais restritos e determinados, a ordem pública ou a paz social ameaçadas por grave e iminente instabilidade institucional ou atingidas por calamidades de grandes proporções na natureza. <strong>Prazo</strong>: até 30 dias, prorrogável uma vez por igual período. <strong>Medidas admitidas</strong>: restrições aos direitos de reunião, sigilo de correspondência e de comunicação telegráfica/telefônica; ocupação e uso temporário de bens e serviços públicos; requisição de bens em caso de calamidade pública. <strong>Controle</strong>: o decreto é submetido ao Congresso Nacional em 24 horas, que decide por maioria absoluta; rejeitado, cessa imediatamente.</p>
<p><strong>Estado de Sítio</strong> (Art. 137 a 139): mais grave que o estado de defesa — decretado pelo Presidente <strong>após</strong> solicitar autorização do Congresso Nacional (maioria absoluta), diferente do estado de defesa. <strong>Hipóteses</strong>: comoção grave de repercussão nacional, ou ineficácia de medida tomada durante estado de defesa; declaração de guerra ou resposta a agressão armada estrangeira. <strong>Prazo</strong>: na 1ª hipótese, até 30 dias por vez, prorrogável mediante nova autorização enquanto persistir a situação; na 2ª hipótese, pode durar todo o tempo da guerra ou agressão. <strong>Medidas admitidas</strong> (mais graves que as do estado de defesa): obrigação de permanência em localidade determinada; restrições à liberdade de imprensa e à liberdade de reunião; busca e apreensão em domicílio; intervenção nas empresas de serviços públicos.</p>
<p><strong>Forças Armadas</strong> (Art. 142): Marinha, Exército e Aeronáutica — instituições nacionais permanentes e regulares, organizadas com base na hierarquia e disciplina, sob a autoridade suprema do Presidente da República, destinadas à defesa da Pátria, à garantia dos poderes constitucionais e, por iniciativa de qualquer destes, à garantia da lei e da ordem (GLO). Pegadinha: as Forças Armadas atuam na GLO por <strong>iniciativa</strong> de um dos Poderes, não de ofício por conta própria — sua atuação em segurança pública interna é subsidiária/excepcional, diferente das Polícias Militares (Art. 144), que fazem o policiamento ostensivo de forma rotineira.</p>`,
    questoes: []
  },
  {
...
<table style="width:100%;border-collapse:collapse;font-size:0.82rem;margin:10px 0 16px;">
  <thead>
    <tr style="background:#0c4a6e;color:#7dd3fc;text-align:left;">
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Situação</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Perfeito?</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Válido?</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Eficaz?</th>
    </tr>
  </thead>
  <tbody>
    <tr style="border-bottom:1px solid #1e293b;"><td style="padding:8px 10px;color:#cbd5e1;">Ato regular publicado</td><td style="padding:8px 10px;color:#94a3b8;">Sim</td><td style="padding:8px 10px;color:#94a3b8;">Sim</td><td style="padding:8px 10px;color:#94a3b8;">Sim</td></tr>
    <tr style="background:#0f172a;border-bottom:1px solid #1e293b;"><td style="padding:8px 10px;color:#cbd5e1;">Ato válido aguardando condição</td><td style="padding:8px 10px;color:#94a3b8;">Sim</td><td style="padding:8px 10px;color:#94a3b8;">Sim</td><td style="padding:8px 10px;color:#94a3b8;">Não</td></tr>
    <tr style="border-bottom:1px solid #1e293b;"><td style="padding:8px 10px;color:#cbd5e1;">Ato com vício já publicado</td><td style="padding:8px 10px;color:#94a3b8;">Sim</td><td style="padding:8px 10px;color:#94a3b8;">Não</td><td style="padding:8px 10px;color:#94a3b8;">Sim (até anulação)</td></tr>
    <tr style="background:#0f172a;"><td style="padding:8px 10px;color:#cbd5e1;">Ato em formação (ainda não concluído)</td><td style="padding:8px 10px;color:#94a3b8;">Não</td><td style="padding:8px 10px;color:#94a3b8;">—</td><td style="padding:8px 10px;color:#94a3b8;">Não</td></tr>
  </tbody>
</table>
<p><strong>Atos inexistentes × atos nulos</strong>: a doutrina distingue uma categoria ainda mais grave do que a nulidade. <strong>Ato nulo</strong> — possui vício em algum dos seus elementos (competência, finalidade, forma, motivo ou objeto), mas tem aparência de ato administrativo; deve ser anulado pela Administração ou pelo Judiciário, e produz efeitos até a declaração de nulidade (<em>ex tunc</em>). <strong>Ato inexistente</strong> — tem apenas a aparência exterior de ato administrativo, mas lhe falta elemento essencial de forma grosseira (exemplo clássico: ato praticado por usurpador de função, alguém que não é agente público de forma alguma); não produz nenhum efeito jurídico, não se convalida, não precisa ser formalmente anulado — simplesmente não existe no mundo jurídico e pode ser ignorado pela Administração e pelos particulares. Diferença prática cobrada em prova: o ato nulo precisa ser formalmente anulado; o ato inexistente pode ser simplesmente desconsiderado.</p>
<p><strong>Silêncio administrativo</strong>: ocorre quando a Administração se omite diante de um requerimento ou situação que exigiria manifestação expressa. <strong>Regra geral</strong>: o silêncio NÃO significa consentimento, deferimento ou indeferimento — salvo quando a lei expressamente atribuir a ele um efeito jurídico determinado. <strong>Consequências do silêncio indevido</strong>: cabe <strong>mandado de segurança</strong> para compelir a autoridade omissa a praticar o ato devido (direito líquido e certo à resposta dentro do prazo legal); em alguns casos a lei prevê prazo específico para manifestação, esgotado o qual o interessado pode buscar tutela judicial por outra via. <strong>Exemplo de silêncio com efeito legal expresso</strong>: a Lei 9.784/99 (Art. 49) fixa prazo de até <strong>30 dias</strong> para decisão, prorrogável por igual período — o descumprimento caracteriza omissão impugnável, mas <strong>não implica deferimento automático</strong>.</p>
<p><strong>Atos políticos (ou de governo)</strong>: praticados pelos Chefes do Poder Executivo (ou por outros agentes políticos) com fundamento direto na Constituição Federal, no exercício de função de governo — não de mera administração. <strong>Características</strong>: fundamentam-se diretamente na CF, não em lei ordinária; possuem ampla margem de liberdade e discricionariedade política; distinguem-se dos atos administrativos comuns por sua natureza eminentemente política. <strong>Exemplos</strong>: veto a projeto de lei, decretação de estado de defesa ou de sítio, declaração de guerra, concessão de indulto, reconhecimento de estado estrangeiro, intervenção federal. <strong>Controle judicial</strong>: bastante restrito — o Judiciário pode controlar os atos políticos quanto à forma e aos pressupostos constitucionais, mas não pode substituir o juízo político de conveniência e oportunidade do agente competente; não se aplica aqui o controle de mérito típico dos atos administrativos discricionários. <strong>Distinção importante para prova</strong>: ato político <strong>não é</strong> sinônimo de ato administrativo discricionário — o ato discricionário tem base em lei e sofre controle mais amplo; o ato político tem base direta na CF e sofre controle mais restrito.</p>
...
<p><strong>Classificação geral</strong>: <strong>agentes políticos</strong> — ocupantes de cargos estruturais do Estado (Presidente, Governadores, Prefeitos, parlamentares, magistrados, membros do Ministério Público), com vínculo de natureza política, não estatutária; <strong>servidores públicos estatutários</strong> — vínculo legal (estatuto), cargo efetivo ou em comissão, regime próprio de previdência; <strong>empregados públicos</strong> — vínculo contratual (CLT), emprego público, regime geral de previdência (RGPS); <strong>servidores temporários</strong> — contratados por prazo determinado para atender a necessidade temporária de excepcional interesse público (Art. 37, IX, CF); <strong>particulares em colaboração com o Estado</strong> — exercem função pública sem vínculo permanente (ex.: mesários, jurados, leiloeiros, delegados de função).</p>
<p><strong>Cargos, empregos e funções públicas</strong>: <strong>cargo público</strong> — criado por lei, com denominação própria, número certo e remuneração pelos cofres públicos, vínculo estatutário; <strong>emprego público</strong> — criado por lei, vínculo celetista (CLT), típico de empresas públicas e sociedades de economia mista; <strong>função pública</strong> — atribuição ou conjunto de atribuições sem cargo ou emprego correspondente (ex.: funções de confiança, exclusivas de servidores efetivos).</p>
...
<ul>
  <li><strong>Concorrência</strong>: contratos de grande vulto — obras, serviços, compras e alienações de alto valor.</li>
  <li><strong>Concurso</strong>: escolha de trabalho técnico, científico ou artístico, com prêmio ou remuneração ao vencedor.</li>
  <li><strong>Leilão</strong>: alienação de bens imóveis ou móveis inservíveis ou apreendidos — critério: maior lance.</li>
  <li><strong>Pregão</strong>: aquisição de bens e serviços comuns (padronizados) — critério: menor preço ou maior desconto, com preferência para a forma eletrônica.</li>
...
<table style="width:100%;border-collapse:collapse;font-size:0.82rem;margin:10px 0 16px;">
  <thead>
    <tr style="background:#0c4a6e;color:#7dd3fc;text-align:left;">
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Entidade</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Criação</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Personalidade</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Regime de pessoal</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Atividade típica</th>
    </tr>
  </thead>
  <tbody>
    <tr style="border-bottom:1px solid #1e293b;"><td style="padding:8px 10px;color:#fde68a;font-weight:700;">Autarquia</td><td style="padding:8px 10px;color:#cbd5e1;">Lei específica cria diretamente</td><td style="padding:8px 10px;color:#94a3b8;">Direito Público</td><td style="padding:8px 10px;color:#94a3b8;">Estatutário</td><td style="padding:8px 10px;color:#cbd5e1;">Atividade típica de Estado</td></tr>
    <tr style="background:#0f172a;border-bottom:1px solid #1e293b;"><td style="padding:8px 10px;color:#fde68a;font-weight:700;">Fundação Pública de Dir. Público</td><td style="padding:8px 10px;color:#cbd5e1;">Lei específica cria diretamente</td><td style="padding:8px 10px;color:#94a3b8;">Direito Público</td><td style="padding:8px 10px;color:#94a3b8;">Estatutário</td><td style="padding:8px 10px;color:#cbd5e1;">Atividades sociais, culturais, científicas</td></tr>
    <tr style="border-bottom:1px solid #1e293b;"><td style="padding:8px 10px;color:#fde68a;font-weight:700;">Fundação Pública de Dir. Privado</td><td style="padding:8px 10px;color:#cbd5e1;">Lei autoriza; registro civil cria</td><td style="padding:8px 10px;color:#94a3b8;">Direito Privado</td><td style="padding:8px 10px;color:#94a3b8;">CLT</td><td style="padding:8px 10px;color:#cbd5e1;">Atividades sociais, culturais, científicas</td></tr>
    <tr style="background:#0f172a;border-bottom:1px solid #1e293b;"><td style="padding:8px 10px;color:#fde68a;font-weight:700;">Empresa Pública</td><td style="padding:8px 10px;color:#cbd5e1;">Lei autoriza; registro cria</td><td style="padding:8px 10px;color:#94a3b8;">Direito Privado</td><td style="padding:8px 10px;color:#94a3b8;">CLT</td><td style="padding:8px 10px;color:#cbd5e1;">Atividade econômica ou serviço público</td></tr>
...
<p><strong>Título I — Disposições Preliminares (Arts. 1º a 4º).</strong> <strong>Âmbito de aplicação</strong>: institui o regime jurídico único dos servidores públicos civis da <strong>União</strong>, das <strong>autarquias</strong> e das <strong>fundações públicas federais</strong> — não se aplica a empregados públicos celetistas de empresas públicas/sociedades de economia mista, nem a servidores estaduais/municipais. <strong>Servidor</strong> (Art. 2º): é a pessoa legalmente investida em cargo público. <strong>Cargo público</strong> (Art. 3º): conjunto de atribuições e responsabilidades cometidas a um servidor, criado por <strong>lei</strong>, com denominação própria, número certo e pago pelos cofres públicos — os cargos públicos são acessíveis aos brasileiros que preencham os requisitos legais (Parágrafo único). <strong>Proibição de serviços gratuitos</strong> (Art. 4º): é <strong>vedada</strong> a prestação de serviços gratuitos, salvo casos previstos em lei.</p>
<p><strong>Requisitos para investidura em cargo público</strong> (Art. 5º): <strong>nacionalidade brasileira</strong> (ressalvados os cargos acessíveis a estrangeiros, nos termos da CF); <strong>gozo dos direitos políticos</strong>; <strong>quitação com as obrigações militares e eleitorais</strong>; <strong>nível de escolaridade</strong> exigido para o exercício do cargo; <strong>idade mínima de dezoito anos</strong>; e <strong>aptidão física e mental</strong>, verificada por junta médica oficial. As atribuições do cargo podem exigir outros requisitos, estabelecidos em lei.</p>
...
<p><strong>Posse e exercício</strong> (Arts. 13 a 15): <strong>posse</strong> é a investidura no cargo público, e deve ocorrer no prazo de <strong>30 dias</strong> contados da publicação do ato de provimento — se o servidor não tomar posse nesse prazo, o ato de provimento é <strong>tornado sem efeito</strong>. <strong>Exercício</strong> é o efetivo desempenho das atribuições do cargo, e deve iniciar no prazo de <strong>15 dias</strong> contados da data da posse — se o servidor não entrar em exercício nesse prazo, é <strong>exonerado de ofício</strong>. Em se tratando de <strong>remoção</strong>, o prazo pra entrar em exercício é de <strong>10 dias</strong> se for na mesma sede do órgão, ou de <strong>30 dias</strong> se implicar mudança de sede/domicílio (Art. 18).</p>
<p><strong>Modalidades de remoção</strong> (Art. 36) — deslocamento do servidor a pedido ou de ofício, com ou sem mudança de sede: <strong>de ofício</strong>, no interesse da Administração; <strong>a pedido, a critério da Administração</strong>, quando depende exclusivamente da conveniência do órgão; <strong>a pedido, independentemente do interesse da Administração</strong>, nas hipóteses de motivo de <strong>saúde</strong> do servidor, cônjuge/companheiro ou dependente que conste do seu assentamento funcional (condicionada à comprovação por junta médica oficial), de <strong>cônjuge/companheiro também servidor público</strong> que foi deslocado no interesse da Administração, ou em virtude de <strong>processo seletivo</strong> promovido conforme normas do órgão/entidade — a remoção por <strong>mandato eletivo</strong> também se enquadra entre as hipóteses independentes do interesse da Administração.</p>
<p><strong>Título III — Direitos e vantagens.</strong> <strong>Vencimento</strong> é a retribuição pecuniária do cargo, fixada em lei; <strong>remuneração</strong> é o vencimento acrescido das vantagens pecuniárias permanentes. <strong>Vantagens</strong>: indenizações (ajuda de custo — mudança de domicílio; diárias — afastamento temporário da sede; transporte; auxílio-moradia), gratificações e adicionais (adicional noturno, adicional por serviço extraordinário, adicional de insalubridade/periculosidade, gratificação natalina/13º). <strong>Ajuda de custo — Art. 57, cônjuges servidores</strong>: quando o cônjuge (ou companheiro) também for servidor público federal e ambos forem transferidos de ofício para a mesma localidade, <strong>apenas um deles fará jus ao recebimento da ajuda de custo</strong>. <strong>Férias</strong>: 30 dias por ano, podendo ser parceladas em até 3 etapas. <strong>Licenças</strong>: por motivo de doença em pessoa da família (com possível prejuízo de remuneração conforme o tempo); para atividade política; para capacitação (após cada quinquênio de efetivo exercício, até 3 meses); para tratar de interesse particular (até 3 anos consecutivos, sem remuneração, a critério da Administração); para desempenho de mandato classista; licença-maternidade (120 dias) e licença-paternidade (5 dias, prorrogável em programas específicos); licença por acidente em serviço.</p>
<p><strong>Concessões</strong> (Art. 97) — o servidor pode ausentar-se do serviço, sem qualquer prejuízo, nos seguintes casos: por <strong>1 dia</strong>, pra doação de sangue; por <strong>2 dias</strong>, pra se alistar como eleitor; por <strong>8 dias consecutivos</strong>, em razão de <strong>casamento</strong> ou de <strong>falecimento</strong> do cônjuge/companheiro, pais, madrasta/padrasto, filhos, enteados, menor sob guarda/tutela ou irmãos.</p>
...
<p><strong>Responsabilidade civil</strong> (Arts. 122 a 125): decorre de <strong>ato omissivo ou comissivo</strong>, doloso ou culposo, praticado no desempenho do cargo ou função, que resulte em <strong>prejuízo ao erário ou a terceiros</strong> — o servidor responde perante a Fazenda Pública em ação regressiva. A obrigação de reparar o dano <strong>estende-se aos sucessores</strong> e contra eles será executada, <strong>até o limite do valor da herança</strong> recebida.</p>
<p><strong>Direito de petição e prescrição</strong> (Arts. 104 a 115 e 142): é assegurado ao servidor o direito de requerer aos Poderes Públicos, em defesa de direito ou interesse legítimo. <strong>Prazos prescricionais</strong> (Art. 142), contados da data em que o fato se tornou conhecido: <strong>5 anos (quinquenal)</strong> para infrações puníveis com demissão, cassação de aposentadoria/disponibilidade, destituição de cargo em comissão e demais atos de <strong>interesse patrimonial</strong>; <strong>2 anos</strong> para suspensão; <strong>180 dias</strong> para advertência. Já os requerimentos e recursos <strong>sem efeito patrimonial direto</strong> prescrevem em <strong>120 dias</strong>, contados da publicação/ciência do ato impugnado. <strong>Prazo decadencial de recursos</strong>: em regra, <strong>30 dias</strong> da ciência da decisão recorrida, salvo prazo diferente estabelecido em lei especial. <strong>Revisão do processo disciplinar</strong>: pode ser requerida <strong>a qualquer tempo</strong>, a pedido ou de ofício, quando forem aduzidos <strong>fatos novos</strong> ou circunstâncias suscetíveis de justificar a inocência do punido ou a inadequação da penalidade aplicada — não pode, porém, resultar em agravamento da penalidade.</p>
...
<p><strong>Título VI — Da Seguridade Social do Servidor.</strong> <strong>Disposições gerais</strong>: a seguridade social do servidor visa dar cobertura aos riscos de doença, invalidez, idade avançada, morte e reclusão, além de proteção à maternidade, à adoção e à paternidade — abrangendo o custeio da assistência à saúde e os benefícios e vantagens previstos no regime próprio.</p>
<p><strong>Benefícios</strong>: <strong>aposentadoria</strong> — por invalidez permanente (proventos proporcionais ao tempo de contribuição, salvo quando decorrente de acidente em serviço, moléstia profissional ou doença grave/contagiosa/incurável especificada em lei, hipóteses em que os proventos são integrais), compulsória (aos 75 anos, com proventos proporcionais ao tempo de contribuição) e voluntária (por idade e/ou tempo de contribuição, conforme os requisitos constitucionais); <strong>pensão por morte</strong>, devida aos dependentes do servidor falecido; <strong>auxílio-natalidade</strong>, devido à servidora (ou ao servidor, se a esposa/companheira não for servidora) por motivo de nascimento de filho, inclusive em caso de natimorto e de adoção; <strong>salário-família</strong>, devido ao servidor com remuneração igual ou inferior ao limite legal, por dependente econômico; <strong>licença para tratamento de saúde</strong>, mediante perícia oficial; <strong>licença à gestante</strong> (120 dias) e <strong>à adotante</strong> (mesmo prazo, conforme a idade da criança adotada); <strong>licença-paternidade</strong> (5 dias, prorrogável em programas específicos); <strong>licença por acidente em serviço</strong>, com remuneração integral; <strong>assistência à saúde</strong>, prestada direta ou indiretamente pelo Poder Público.</p>
<p><strong>Assistência à saúde</strong>: compreende ações preventivas e de promoção da saúde, assistência ambulatorial, hospitalar e odontológica, prestada pelo Sistema Único de Saúde (SUS), diretamente pelo órgão ou entidade, ou mediante convênio, contrato ou credenciamento com operadoras de planos de saúde.</p>
<p><strong>Custeio</strong>: a seguridade social do servidor é financiada pela União (como ente patronal) e pelos servidores ativos, aposentados e pensionistas, mediante contribuição compulsória incidente sobre a remuneração, os proventos e as pensões, nos termos fixados em lei.</p>
<p><strong>Título VII — Da Contratação Temporária de Excepcional Interesse Público.</strong> <strong>Hipóteses</strong>: a contratação por tempo determinado (regida pela Lei 8.745/93) é admitida para atender a necessidade temporária de excepcional interesse público, como calamidade pública, emergências em saúde pública, combate a surtos/epidemias, recenseamentos, admissão de professores e pesquisadores visitantes, atividades de caráter emergencial em órgãos de pesquisa, entre outras hipóteses taxativamente previstas em lei — <strong>não</strong> se aplica a qualquer necessidade transitória de pessoal, apenas às situações expressamente listadas.</p>
<p><strong>Prazo</strong>: em regra, até 2 anos, admitida prorrogação, variando conforme a hipótese específica prevista na Lei 8.745/93 (alguns casos, como calamidade pública, têm prazo de até 6 meses).</p>
<p><strong>Regime jurídico do contratado temporário</strong>: é regido por contrato administrativo de natureza especial (não é regido integralmente pela Lei 8.112/90), submetendo-se a um regime jurídico-administrativo próprio, com aplicação subsidiária de dispositivos da Lei 8.112/90 apenas quando expressamente previsto; vinculado ao Regime Geral de Previdência Social (RGPS), e não ao regime próprio dos servidores estatutários.</p>
<p><strong>Contratado temporário × servidor efetivo</strong>: o contratado temporário <strong>não é servidor estatutário</strong> — não adquire estabilidade, não se submete a concurso público (a contratação pode ser precedida de processo seletivo simplificado), seu vínculo é contratual e por prazo determinado (extingue-se automaticamente ao final do prazo, sem necessidade de exoneração), e ele não integra o Regime Jurídico Único da Lei 8.112/90, ao contrário do servidor efetivo, que ingressa por concurso público, tem vínculo estatutário por prazo indeterminado e, após o estágio probatório, adquire estabilidade.</p>`,
    questoes: []
  },
  {
...
<p><strong>Requisitos de elegibilidade para dirigentes</strong> (Art. 12 e 13): para <strong>Diretor-Geral de campus</strong>, o candidato deve, cumulativamente, ser servidor ocupante de cargo efetivo da carreira docente ou de nível superior da carreira dos técnico-administrativos (PCCTAE) e ter no mínimo <strong>5 anos de efetivo exercício</strong> em instituição federal de educação profissional e tecnológica — além disso, deve preencher ao menos <strong>uma</strong> destas condições adicionais: preencher os requisitos exigidos para Reitor; ou ter ao menos <strong>2 anos de experiência</strong> em cargo ou função de gestão; ou ter concluído curso de formação para gestão pública. Para <strong>Reitor</strong>, o candidato deve ser docente da carreira de magistério do ensino básico, técnico e tecnológico (EBTT) ou do magistério superior, estar em efetivo exercício na instituição há pelo menos <strong>5 anos</strong>, e possuir o mínimo de titulação ou enquadramento em classes específicas definidas na legislação da carreira docente federal.</p>
<p><strong>Processo de consulta à comunidade e princípio da paridade</strong> (Art. 12): a escolha do Reitor e dos Diretores-Gerais de campus ocorre por meio de <strong>consulta à comunidade escolar</strong>, adotando obrigatoriamente o regime de <strong>paridade tripartite</strong> — o peso total da votação é dividido em três partes iguais:</p>
<ul>
  <li><strong>1/3 do peso</strong> → manifestação do corpo docente</li>
  <li><strong>1/3 do peso</strong> → manifestação dos servidores técnico-administrativos</li>
  <li><strong>1/3 do peso</strong> → manifestação do corpo discente (estudantes)</li>
</ul>
<p><strong>Ponto crítico para prova</strong>: a paridade é de <strong>PESO</strong>, não de número de votantes. Mesmo que uma categoria tenha muito mais membros que outra, cada segmento vale exatamente 1/3 do resultado final. O resultado da consulta é encaminhado ao <strong>Ministro da Educação</strong>, a quem compete a nomeação.</p>
<p><strong>Composição da Reitoria</strong>: a Reitoria é composta por <strong>1 Reitor</strong> e <strong>5 Pró-Reitores</strong>, nomeados pelo Reitor, que o auxiliam na gestão das áreas acadêmica, administrativa, de extensão, de pesquisa e de desenvolvimento institucional, conforme a estrutura de cada Instituto Federal.</p>
<table style="width:100%;border-collapse:collapse;font-size:0.82rem;margin:10px 0 16px;">
  <thead>
    <tr style="background:#0c4a6e;color:#7dd3fc;text-align:left;">
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Órgão</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Caráter</th>
      <th style="padding:8px 10px;border-bottom:2px solid #334155;">Composição base</th>
    </tr>
  </thead>
  <tbody>
    <tr style="border-bottom:1px solid #1e293b;"><td style="padding:8px 10px;color:#fde68a;font-weight:700;">Conselho Superior</td><td style="padding:8px 10px;color:#94a3b8;">Consultivo E deliberativo</td><td style="padding:8px 10px;color:#cbd5e1;">Representantes de servidores docentes, técnico-administrativos, estudantes, egressos, sociedade civil e Poder Executivo</td></tr>
    <tr style="background:#0f172a;"><td style="padding:8px 10px;color:#fde68a;font-weight:700;">Colégio de Dirigentes</td><td style="padding:8px 10px;color:#94a3b8;">Exclusivamente consultivo</td><td style="padding:8px 10px;color:#cbd5e1;">Reitor (presidente) + Pró-Reitores + Diretores-Gerais de campus</td></tr>
  </tbody>
</table>
<p><strong>Ponto crítico para prova</strong>: o Colégio de Dirigentes é <strong>APENAS consultivo</strong> — nunca deliberativo. O Conselho Superior acumula as duas funções (consultivo e deliberativo).</p>
<p><strong>Conceito de aluno-equivalente</strong> (Art. 8º, §1º): o cumprimento da proporção mínima de <strong>50% das vagas</strong> destinadas a cursos técnicos de nível médio (prioritariamente na forma integrada) é verificado pelo conceito de <strong>aluno-equivalente</strong>, regulamentado por normas do Ministério da Educação — e não pelo número bruto de matrículas. <strong>Aluno-equivalente</strong> é a metodologia de cálculo que pondera a carga horária e o nível do curso, permitindo comparar cursos de naturezas e durações diferentes numa mesma base numérica. A oferta pode ser ajustada com anuência do MEC sem perda da proporção mínima exigida, desde que a equivalência seja mantida no cálculo global. <strong>Ponto crítico para prova</strong>: a proporção de 50% é calculada em alunos-equivalentes, não em número de vagas absolutas — o que permite flexibilidade na composição da oferta.</p>
<p><strong>Disposições transitórias e processo de implantação</strong> (Art. 14 a 18): os dirigentes das instituições anteriores (CEFETs, Escolas Técnicas Federais, Escolas Agrotécnicas Federais) que foram transformadas em Institutos Federais assumiram automaticamente como <strong>Reitores ou Diretores-Gerais <em>pro tempore</em></strong>, com mandato que se estende até o término do mandato original para o qual foram eleitos ou designados — sem interrupção. O <strong>Reitor <em>pro tempore</em></strong> tem prazo máximo de <strong>180 dias</strong>, contados da publicação da lei, para elaborar e encaminhar ao MEC a proposta de <strong>Estatuto</strong> do Instituto Federal e o <strong>Plano de Desenvolvimento Institucional (PDI)</strong> — o descumprimento desse prazo caracteriza omissão administrativa relevante para fins de controle institucional. Os <strong>bens, direitos e obrigações</strong> das instituições anteriores (imóveis, equipamentos, acervos, contratos em andamento e quadro de pessoal) são transferidos automaticamente para os novos Institutos Federais, sem necessidade de qualquer formalidade adicional, sem reservas e sem condições. <strong>Ponto crítico para prova</strong>: a transferência patrimonial é <strong>automática e incondicional</strong> — não depende de escritura, registro ou ato administrativo específico de transferência.</p>
<p><strong>Concessão de bolsas</strong>: os Institutos Federais possuem previsão legal expressa para conceder bolsas a: <strong>alunos</strong> regularmente matriculados na instituição; <strong>docentes e pesquisadores</strong> vinculados ao IF; e <strong>pesquisadores externos ou de empresas parceiras</strong>. As bolsas podem ser destinadas a atividades de <strong>pesquisa</strong>, <strong>desenvolvimento tecnológico</strong>, <strong>inovação</strong> e <strong>intercâmbio nacional e internacional</strong>. A concessão obedece à regulamentação do Ministério da Educação. <strong>Ponto crítico para prova</strong>: a previsão de bolsas para pesquisadores de empresas externas é um diferencial dos IFs em relação a outras instituições federais de ensino — reforça o papel de integração com o setor produtivo.</p>
<p><strong>Diretrizes de expansão</strong>: a criação de novas instituições federais de educação profissional e tecnológica, bem como a expansão das já existentes, deve obrigatoriamente <strong>obedecer ao modelo de Instituto Federal</strong> definido pela Lei nº 11.892/2008 e <strong>seguir as normas e diretrizes estabelecidas pelo Ministério da Educação</strong>. <strong>Ponto crítico para prova</strong>: a expansão não é livre — está vinculada ao modelo IF e às normas do MEC. Novas unidades não podem ser criadas fora desse modelo sem autorização legislativa específica.</p>
<p><strong>Autonomia de cursos</strong>: a autonomia didático-pedagógica dos Institutos Federais compreende o poder de <strong>planejar, organizar e ofertar</strong> os cursos previstos em lei, e <strong>definir metodologias, currículos e formas de avaliação</strong>, respeitadas as diretrizes curriculares nacionais. Essa autonomia tem limites expressos em lei — os IFs devem obrigatoriamente respeitar: <strong>mínimo de 50%</strong> das vagas para cursos técnicos de nível médio (preferencialmente na forma integrada), calculado em aluno-equivalente; e <strong>mínimo de 20%</strong> das vagas para cursos de licenciatura e programas especiais de formação pedagógica, destinados à formação de professores para a educação básica e à educação profissional. <strong>Ponto crítico para prova</strong>: a autonomia didático-pedagógica NÃO é irrestrita — está subordinada ao cumprimento obrigatório dos percentuais mínimos de 50% e 20%, que são imposições legais, não metas sugeridas.</p>

<h3>Colégio Pedro II e Escolas Técnicas Vinculadas</h3>
<p>O <strong>Colégio Pedro II</strong> é uma instituição federal de ensino pluricurricular e multicampi, com sede no Rio de Janeiro. A Lei nº 11.892/2008 trata expressamente de sua <strong>equiparação</strong> aos Institutos Federais, sem transformá-lo em um IF.</p>
<ul>
<li><strong>Equiparação expressa</strong>: o Colégio Pedro II é equiparado aos Institutos Federais para fins de: autonomia administrativa, financeira e didático-pedagógica; gestão de pessoal; e regulação, avaliação e supervisão pelo MEC.</li>
<li><strong>Criação de novos campi</strong>: a expansão do Colégio Pedro II mediante criação de novos campi depende de <strong>autorização específica do Ministério da Educação</strong> — não é ato autônomo da instituição.</li>
<li><strong>Escolas Técnicas Vinculadas às Universidades Federais</strong>: permanecem como unidades de ensino vinculadas às universidades federais, <strong>não sendo transformadas em Institutos Federais</strong>. Sua regulação segue as normas aplicáveis às universidades federais às quais estão vinculadas.</li>
</ul>
<p><strong>Ponto crítico para prova</strong>: o Colégio Pedro II <strong>NÃO é um Instituto Federal</strong> — é equiparado a eles para fins específicos previstos em lei. A distinção é cobrada diretamente pelas bancas.</p>

<h3>Detalhamento Estatutário dos Órgãos Colegiados</h3>
<p>A Lei nº 11.892/2008 define a existência e a natureza do Conselho Superior e do Colégio de Dirigentes, mas estabelece expressamente que a estruturação detalhada de cada órgão colegiado, suas competências específicas e as normas de funcionamento (periodicidade de reuniões, quórum, deliberação, recursos) devem constar do <strong>Estatuto próprio de cada Instituto Federal</strong>.</p>
<p><strong>O que a lei define diretamente</strong> (sem remeter ao Estatuto):</p>
<ul>
<li><strong>Conselho Superior</strong>: caráter consultivo E deliberativo.</li>
<li><strong>Colégio de Dirigentes</strong>: caráter exclusivamente consultivo.</li>
<li>Composição mínima de cada órgão.</li>
<li>Competência geral de cada órgão.</li>
</ul>
<p><strong>Ponto crítico para prova</strong>: a lei fixa a natureza e a composição mínima dos órgãos; o Estatuto de cada IF complementa com as regras operacionais. Questões que atribuem ao Colégio de Dirigentes caráter deliberativo estão <strong>erradas</strong> — é exclusivamente consultivo.</p>

<h3>Competência de Registro e Expedição de Diplomas</h3>
<p>Os Institutos Federais possuem <strong>plena autonomia</strong> para expedir os diplomas dos cursos que ofertam — técnicos, de graduação, de pós-graduação lato sensu e stricto sensu — e para registrar os diplomas expedidos por suas próprias unidades, exercendo papel central na validação e certificação de saberes profissionais no âmbito da educação profissional e tecnológica.</p>
<p><strong>Abrangência da competência de certificação</strong>: cursos técnicos de nível médio; cursos de graduação (tecnólogos, licenciaturas e bacharelados); cursos de pós-graduação lato sensu (especializações); e cursos de pós-graduação stricto sensu (mestrado e doutorado), quando autorizados.</p>
<p><strong>Ponto crítico para prova</strong>: a autonomia para expedir E registrar diplomas é <strong>plena</strong> — os IFs não dependem de universidades ou de outros órgãos para validar os diplomas de seus próprios cursos.</p>

<h3>Regras sobre Criação de Novos Campi e Expansão</h3>
<p>A expansão dos Institutos Federais mediante criação de novos campi <strong>não é ato discricionário autônomo</strong> da instituição. A lei estabelece que:</p>
<ul>
<li>A criação de novas unidades deve obedecer a parâmetros rigorosos definidos pelo Ministério da Educação.</li>
<li>O modelo multicampi deve seguir diretrizes estruturadas e nacionais fixadas pelo MEC.</li>
<li>A expansão está condicionada ao atendimento de critérios técnicos, orçamentários e de demanda social definidos pelo MEC.</li>
<li>Novas unidades criadas fora do modelo de Instituto Federal exigem autorização legislativa específica.</li>
</ul>
<p><strong>Relação com as Diretrizes de Expansão</strong>: a expansão da Rede Federal de Educação Profissional, Científica e Tecnológica como um todo obedece a planos nacionais de expansão coordenados pelo MEC. Os IFs participam dessa expansão por meio da criação de novos campi, sempre dentro dos parâmetros legais e das diretrizes do MEC.</p>
<p><strong>Ponto crítico para prova</strong>: a autonomia dos IFs <strong>NÃO inclui</strong> a criação livre de novos campi — essa expansão depende de autorização e parâmetros do MEC. Autonomia didático-pedagógica e autonomia para expandir fisicamente são institutos distintos.</p>`,
    questoes: []
  },
  {
